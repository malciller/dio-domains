...
Every pass logs one human-readable block per asset (indented continuation lines): the headline ("ACTIVE — buy 0.15 ETH on every 5.00% price drop"), the capital line ("capital: share $854.21 of the $864.27 venue pool · deployed $854.21 · passes $0.00 down"), the sizing drawdown with the actual peak→valley event and dates, the replayed survival ("capital runs out after a 9.7% price drop (target 95%)"), and - for crypto - the gi blend composition ("gi blend: fng 37.00 -> 2.32% (w 0.50) | range 2.32% (w 0.25) | survival 5.00% (w 0.25) -> resolved 5.00% (blend 2.99% clamped: survival binds)") and the qty blend ("qty blend: floor 0.15 -> resolved 0.15 (fng 37.00, k 0.63; no headroom: F&G qty contribution 0)") so the weighting and any clamp-away of sentiment is visible, not silent. Equities log a single "sizing: pure oracle" line instead. The F&G/runtime blend is recomputed over the same config range only as the no-decision fallback.
