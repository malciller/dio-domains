...
let close_hedged_positions () =
  ...
  then Lwt.return_unit
    ...
    Logging.info
      ~section:"main"
      "Fetching active Hyperliquid Perp positions for graceful shutdown...";
    let wallet =
      ...
      | None ->
        Logging.warn
          ~section:"main"
          "HYPERLIQUID_WALLET_ADDRESS not set, cannot close hedges!";
        ""
    in
    if wallet = ""
    then Lwt.return_unit
  ...
  let handle_graceful_shutdown _signum =
    ...
    then (
      ...
      Lwt.async (fun () ->
        ...
        Logging.info
          ~section:"main"
          "Shutdown cleanup initiated, force exit available with second Ctrl+C...";
        Lwt.return_unit))
    else if not (Atomic.get force_exit_requested)
    then (
...
let setup_fatal_signal_handlers () =
  let handle_fatal_signal signum =
    let signal_name =
      ...
      | 7 -> "SIGBUS (bus error)"
      | 10 ->
        "SIGBUS/SIGUSR1 (bus error or user signal)"
        (* Signal 10 maps to SIGBUS or SIGUSR1 depending on platform. *)
    ...
    Logging.critical_f
      ~section:"main"
      "Memory: heap=%dMB live=%dMB free=%dMB fragments=%d compactions=%d"
    ...
    Logging.critical_f
      ~section:"main"
      "Fatal signal received: %s - attempting emergency shutdown"
      signal_name;
    ...
    Logging.critical_f
      ~section:"main"
      "Process terminating due to fatal signal: %s"
      signal_name;
...
let setup_lwt_exception_handler () =
  ...
  := fun exn ->
       ...
       Logging.critical_f
         ~section:"main"
         "Unhandled exception in Lwt.async: %s\nBacktrace:\n%s"
         (Printexc.to_string exn)
         backtrace;
...
let () =
    ...
    Lwt.async (fun () ->
      let rec loop () =
        ...
        then Lwt.return_unit
          ...
          Logging.info_f
            ~section:"memory"
            "heap=%dMB live=%dKB free=%dKB runtime=%.0fs gc_major=%d chunks=%d"
            heap_mb
            live_kb
            free_kb
            runtime
  ...
  (try
   ...
   | exn ->
     Logging.error_f
       ~section:"main"
       "Failed to initialize Conduit context: %s"
       (Printexc.to_string exn);
     raise exn);
  ...
  try
    ...
    (try Lwt_main.run (Lwt_condition.wait shutdown_condition) with
     | e ->
       let backtrace = Printexc.get_backtrace () in
       Logging.critical_f
         ~section:"main"
         "Fatal exception in Lwt event loop: %s\nBacktrace:\n%s"
    ...
    | Some signal_num ->
      Logging.critical_f
        ~section:"main"
        "Process terminating due to fatal signal %d received during execution"
        signal_num;
      exit 1
    | None ->
  ...
  | e ->
    ...
    Logging.critical_f
      ~section:"main"
      "Fatal exception during startup: %s\nBacktrace:\n%s"
      (Printexc.to_string e)
      backtrace;
    exit 1
;;
