let test_level_structure () =
  let level = { Ibkr.Orderbook_feed.price = 175.50; size = 1000.0 } in
...
let test_orderbook_structure () =
  ...
  let ob = { Ibkr.Orderbook_feed.bids; asks; timestamp = Unix.gettimeofday () } in
...
let test_store_depth () =
  ...
  Alcotest.(check int)
    "store depth matches default"
    Ibkr.Types.default_orderbook_depth
    store.depth;
  Alcotest.(check int)
    "bids array length"
    Ibkr.Types.default_orderbook_depth
    (Array.length store.bids);
  Alcotest.(check int)
    "asks array length"
    Ibkr.Types.default_orderbook_depth
    (Array.length store.asks)
;;

let test_read_events_unknown () =
  let events = Ibkr.Orderbook_feed.read_orderbook_events "IBKR_OB_UNKNOWN" 0 in
  Alcotest.(check (list int))
    "no events for unknown symbol"
    []
    (List.map (fun _ -> 1) events)
;;

let test_get_position_unknown () =
  Alcotest.(check int)
    "position 0 for unknown"
    0
