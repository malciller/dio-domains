...
let test_fold_open_orders_empty () =
  let result =
    Ibkr.Executions_feed.fold_open_orders "IBKR_FOLD_TEST" ~init:0 ~f:(fun acc _oo ->
      acc + 1)
  in
...
let () =
  Alcotest.run
    "IBKR Executions Feed"
    [ ( "data structures"
      , [ Alcotest.test_case
            "execution_event_structure"
            `Quick
    ...
    ; ( "order tracking"
      , [ Alcotest.test_case
            "register_and_resolve_order"
            `Quick
            test_register_and_resolve_order
