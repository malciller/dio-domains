...
let test_handler_overwrite () =
  ...
  Ibkr.Dispatcher.register_handler ~msg_id:9998 ~handler:(fun _fields -> call_count := 1);
  Ibkr.Dispatcher.register_handler ~msg_id:9998 ~handler:(fun _fields -> call_count := 2);
...
let test_req_handler_registration () =
  ...
  let _condition =
    Ibkr.Dispatcher.register_req_handler
      ~req_id:7777
...
let test_handler_receives_correct_fields () =
  ...
  Ibkr.Dispatcher.register_handler ~msg_id:9997 ~handler:(fun fields ->
    captured_fields := fields);
  Ibkr.Dispatcher.dispatch ~msg_id:9997 ~fields:[ "field1"; "field2"; "field3" ];
  Alcotest.(check (list string))
    "handler receives correct fields"
  ...
  Alcotest.run
    ...
    [ ( "handler registration"
      , [ Alcotest.test_case "handler_registration" `Quick test_handler_registration
        ...
        ; Alcotest.test_case
            "handler_receives_correct_fields"
            `Quick
            test_handler_receives_correct_fields
        ] )
    ; ( "dispatch"
      , [ Alcotest.test_case "unhandled_message" `Quick test_unhandled_message
        ...
        ; Alcotest.test_case
            "handler_exception_recovery"
            `Quick
            test_handler_exception_recovery
        ] )
    ; ( "req handlers"
      , [ Alcotest.test_case
            "req_handler_registration"
            `Quick
            test_req_handler_registration
