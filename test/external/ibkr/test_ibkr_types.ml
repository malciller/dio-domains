...
let test_make_limit_order () =
  let o =
    Ibkr.Types.make_limit_order ~order_id:99 ~action:"SELL" ~qty:50.0 ~price:150.25
  in
...
let test_feed_defaults () =
  Alcotest.(check int)
    "default_ring_buffer_size_ticker"
    100
  ...
  Alcotest.(check (float 0.1))
    "default_stale_order_threshold_s"
    86400.0
    Ibkr.Types.default_stale_order_threshold_s;
  Alcotest.(check (float 0.1))
    "default_reconnect_base_delay_ms"
    1000.0
    Ibkr.Types.default_reconnect_base_delay_ms;
  Alcotest.(check (float 0.1))
    "default_reconnect_max_delay_ms"
    30000.0
    Ibkr.Types.default_reconnect_max_delay_ms;
  Alcotest.(check (float 0.01))
    "default_reconnect_backoff_factor"
    2.0
    Ibkr.Types.default_reconnect_backoff_factor
;;
  ...
  Alcotest.run
    ...
    ; ( "order status"
      , [ Alcotest.test_case "parse_tws_order_status" `Quick test_parse_tws_order_status
        ...
        ; Alcotest.test_case
            "to_exchange_order_status"
            `Quick
            test_to_exchange_order_status
    ...
    ; ( "constants"
      , [ Alcotest.test_case "outbound_message_ids" `Quick test_message_id_constants
        ; Alcotest.test_case
            "inbound_message_ids"
            `Quick
            test_inbound_message_id_constants
