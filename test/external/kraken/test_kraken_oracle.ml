(* Kraken oracle: balance parsing (asset normalization, wallet-suffix stripping). Pure
...
let test_parse_balances_normalizes_assets () =
  ...
  | Ok triples ->
    (* XXBT -> BTC; BTC.HOLD (staking) keeps BTC after suffix stripping; ZUSD -> USD. Both
       BTC entries survive; the snapshot layer merges them. *)
