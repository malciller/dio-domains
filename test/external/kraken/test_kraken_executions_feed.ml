...
let test_side_conversions () =
  ...
  in
  let sell_str =
    Kraken.Kraken_executions_feed.string_of_side Kraken.Kraken_executions_feed.Sell
  in
  ...
  Alcotest.(check bool)
    "buy string to side"
    true
    (buy_back = Kraken.Kraken_executions_feed.Buy);
  Alcotest.(check bool)
    "sell string to side"
    true
    (sell_back = Kraken.Kraken_executions_feed.Sell);
  Alcotest.(check bool)
    "invalid side defaults to buy"
    true
    (invalid_side = Kraken.Kraken_executions_feed.Buy)
;;
...
let test_ring_buffer_operations () =
  ...
  Alcotest.(check bool)
    "buffer still contains 123 after wrap"
    true
    (List.mem 123 data_after_wrap);
  Alcotest.(check bool)
    "buffer still contains 999 after wrap"
    true
    (List.mem 999 data_after_wrap);
  ...
  Alcotest.(check bool)
    "buffer no longer contains overwritten 42"
    false
    (List.mem 42 data_after_wrap)
;;
...
let test_execution_event_structure () =
  ...
  Alcotest.(check bool)
    "execution event exec_type"
    true
    (test_event.exec_type = Kraken.Kraken_executions_feed.New);
  Alcotest.(check bool)
    "execution event order_status"
    true
    (test_event.order_status = Kraken.Kraken_executions_feed.NewStatus);
  Alcotest.(check bool)
    "execution event side"
    true
    (test_event.side = Kraken.Kraken_executions_feed.Buy);
  ...
  Alcotest.(check (option (float 0.1)))
    "execution event limit_price"
    (Some 45000.0)
    test_event.limit_price;
  Alcotest.(check bool)
    "execution event last_qty is none"
    true
    (Option.is_none test_event.last_qty);
  Alcotest.(check bool)
    "execution event last_price is none"
    true
    (Option.is_none test_event.last_price);
  Alcotest.(check bool) "execution event fee is none" true (Option.is_none test_event.fee);
  Alcotest.(check bool)
    "execution event trade_id is none"
    true
    (Option.is_none test_event.trade_id);
  Alcotest.(check (option int))
    "execution event order_userref"
    (Some 1)
    test_event.order_userref;
  Alcotest.(check bool)
    "execution event timestamp is positive"
    true
    (test_event.timestamp > 0.0)
;;
...
let test_open_order_structure () =
  ...
  Alcotest.(check bool)
    "open order side"
    true
    (test_order.side = Kraken.Kraken_executions_feed.Sell);
  ...
  Alcotest.(check bool)
    "open order order_status"
    true
    (test_order.order_status = Kraken.Kraken_executions_feed.PartiallyFilledStatus);
  Alcotest.(check (option int))
    "open order order_userref"
    (Some 67890)
    test_order.order_userref;
  Alcotest.(check bool)
    "open order last_updated is positive"
    true
    (test_order.last_updated > 0.0)
;;
...
let test_ring_buffer_capacity () =
  ...
  Alcotest.(check bool)
    "buffer contains new element 4"
    true
    (List.mem 4 data_after_overflow);
  Alcotest.(check bool)
    "buffer no longer contains oldest element 1"
    false
