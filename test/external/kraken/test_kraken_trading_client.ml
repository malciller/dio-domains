let test_string_contains () =
  ...
  Alcotest.(check bool)
    "contains 'world'"
    true
    (Kraken.Kraken_trading_client.string_contains "hello world" "world");
  Alcotest.(check bool)
    "contains 'hello'"
    true
    (Kraken.Kraken_trading_client.string_contains "hello world" "hello");
  Alcotest.(check bool)
    "does not contain 'goodbye'"
    false
    (Kraken.Kraken_trading_client.string_contains "hello world" "goodbye");
  Alcotest.(check bool)
    "does not contain longer substring"
    false
    (Kraken.Kraken_trading_client.string_contains "hi" "hello");
  Alcotest.(check bool)
    "contains single char"
    true
    (Kraken.Kraken_trading_client.string_contains "aaa" "a");
  Alcotest.(check bool)
    "empty string does not contain char"
    false
    (Kraken.Kraken_trading_client.string_contains "" "a")
;;
...
let test_parse_ws_response () =
  ...
  Alcotest.(check string)
    "method field"
    "add_order"
    response.Kraken.Kraken_common_types.method_;
...
let test_connection_status_types () =
  ...
  | _ -> Alcotest.fail "Connection status types test failed"
;;
...
let () =
  Alcotest.run
    ...
    ; ( "field detection"
      , [ Alcotest.test_case
            "precision_field_detection"
            `Quick
            test_precision_field_detection
        ] )
    ; ( "connection types"
