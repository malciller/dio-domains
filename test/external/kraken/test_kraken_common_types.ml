...
let test_normalize_base64_secret () =
  ...
  let test_cases =
    ...
    ; (* Missing padding *)
      "SGVsbG8gV29ybGQ-", "SGVsbG8gV29ybGQ+"
    ; (* URL-safe dash *)
      "SGVsbG8gV29ybGQ_", "SGVsbG8gV29ybGQ/"
    ; (* URL-safe underscore *)
      "  SGVsbG8gV29ybGQ=  ", "SGVsbG8gV29ybGQ="
    ; (* With whitespace *)
  ...
  let test_case secret expected_prefix =
    ...
    | Failure msg ->
      ...
      then () (* Success *)
        ...
        Alcotest.fail
          (Printf.sprintf
             "Expected error starting with '%s', got '%s'"
             expected_prefix
             msg)
  in
...
let test_order_result_structures () =
  ...
  in
  let cancel_result =
    { Kraken.Kraken_common_types.order_id = "order123"; cl_ord_id = Some "client123" }
  in
...
let () =
  Alcotest.run
    "Kraken Common Types"
    [ "nonce", [ Alcotest.test_case "generation" `Quick test_nonce_generation ]
    ; ( "base64"
