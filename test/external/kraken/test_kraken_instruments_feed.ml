...
let test_is_tradeable () =
  ...
  Alcotest.(check bool)
    "Online is tradeable"
    true
    (Kraken.Kraken_instruments_feed.is_tradeable Kraken.Kraken_instruments_feed.Online);
  Alcotest.(check bool)
    "CancelOnly is not tradeable"
    false
    (Kraken.Kraken_instruments_feed.is_tradeable
       Kraken.Kraken_instruments_feed.CancelOnly);
  Alcotest.(check bool)
    "Delisted is not tradeable"
    false
    (Kraken.Kraken_instruments_feed.is_tradeable Kraken.Kraken_instruments_feed.Delisted);
  Alcotest.(check bool)
    "LimitOnly is not tradeable"
    false
    (Kraken.Kraken_instruments_feed.is_tradeable Kraken.Kraken_instruments_feed.LimitOnly);
  Alcotest.(check bool)
    "Maintenance is not tradeable"
    false
    (Kraken.Kraken_instruments_feed.is_tradeable
       Kraken.Kraken_instruments_feed.Maintenance);
  Alcotest.(check bool)
    "PostOnly is not tradeable"
    false
    (Kraken.Kraken_instruments_feed.is_tradeable Kraken.Kraken_instruments_feed.PostOnly);
  Alcotest.(check bool)
    "ReduceOnly is not tradeable"
    false
    (Kraken.Kraken_instruments_feed.is_tradeable
       Kraken.Kraken_instruments_feed.ReduceOnly);
  Alcotest.(check bool)
    "WorkInProgress is not tradeable"
    false
    (Kraken.Kraken_instruments_feed.is_tradeable
       Kraken.Kraken_instruments_feed.WorkInProgress);
  Alcotest.(check bool)
    "Unknown is not tradeable"
    false
    (Kraken.Kraken_instruments_feed.is_tradeable Kraken.Kraken_instruments_feed.Unknown)
;;
...
let test_parse_pair_info () =
  ...
  try
    ...
    | Some info ->
      ...
      Alcotest.(check bool)
        "parsed status is Online"
        true
        (info.status = Kraken.Kraken_instruments_feed.Online);
      Alcotest.(check int) "parsed qty_precision" 8 info.qty_precision;
      Alcotest.(check (float 0.00000001))
        "parsed qty_increment"
        0.00000001
        info.qty_increment;
  ...
  with
  | _ -> Alcotest.fail "exception during parsing"
;;
...
let test_parse_pair_info_invalid () =
  ...
  with
  | _ -> Alcotest.fail "exception during invalid JSON parsing"
;;
...
let test_pair_info_structure () =
  ...
  Alcotest.(check bool)
    "pair_info status is Online"
    true
    (test_info.status = Kraken.Kraken_instruments_feed.Online);
  Alcotest.(check int) "pair_info qty_precision" 8 test_info.qty_precision;
  Alcotest.(check (float 0.00000001))
    "pair_info qty_increment"
    0.00000001
    test_info.qty_increment;
  ...
  Alcotest.(check bool)
    "pair_info last_updated positive"
    true
    (test_info.last_updated > 0.0)
;;

let () =
  Alcotest.run
    "Kraken Instruments Feed"
    [ ( "status handling"
