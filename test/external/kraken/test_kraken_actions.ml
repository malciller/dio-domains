let test_string_contains () =
  ...
  Alcotest.(check bool)
    "contains 'world'"
    true
    (Kraken.Kraken_actions.string_contains "hello world" "world");
  Alcotest.(check bool)
    "contains 'hello'"
    true
    (Kraken.Kraken_actions.string_contains "hello world" "hello");
  Alcotest.(check bool)
    "does not contain 'goodbye'"
    false
    (Kraken.Kraken_actions.string_contains "hello world" "goodbye");
  Alcotest.(check bool)
    "does not contain longer substring"
    false
    (Kraken.Kraken_actions.string_contains "hi" "hello");
  Alcotest.(check bool)
    "contains single char"
    true
    (Kraken.Kraken_actions.string_contains "aaa" "a");
  Alcotest.(check bool)
    "empty string does not contain char"
    false
    (Kraken.Kraken_actions.string_contains "" "a")
;;
...
let test_is_retriable_error () =
  ...
  Alcotest.(check bool)
    ...
    true
    (Kraken.Kraken_actions.is_retriable_error "Connection timeout");
  Alcotest.(check bool)
    "Network error 500 is retriable"
    true
    (Kraken.Kraken_actions.is_retriable_error "Network error 500");
  Alcotest.(check bool)
    "WebSocket connection reset is retriable"
    true
    (Kraken.Kraken_actions.is_retriable_error "WebSocket connection reset");
  Alcotest.(check bool)
    "Invalid order parameters is not retriable"
    false
    (Kraken.Kraken_actions.is_retriable_error "Invalid order parameters");
  Alcotest.(check bool)
    "Insufficient funds is not retriable"
    false
    (Kraken.Kraken_actions.is_retriable_error "Insufficient funds")
;;
...
let test_add_to_assoc () =
  ...
  | `Assoc pairs ->
    let has_existing =
      List.exists (fun (k, v) -> k = "existing" && v = `String "value") pairs
    in
...
let test_retry_with_backoff_logic () =
  ...
  let test_function () =
    incr call_count;
    if !call_count < 3
    then Lwt.return (Error "Temporary failure")
    else Lwt.return (Ok "Success")
  in
  let config =
    { Error_handling.max_attempts = 5
    ; base_delay_ms = 1.0
    ; max_delay_ms = 100.0
    ; backoff_factor = 1.5
    }
...
let test_max_retry_attempts () =
  ...
  in
  match result with
  | Error _ -> Alcotest.(check int) "function called max attempts times" 2 !call_count
...
let () =
  Alcotest.run
    ...
    ; "request handling", [ Alcotest.test_case "next_req_id" `Quick test_next_req_id ]
    ; ( "error handling"
    ...
    ; ( "configuration"
      , [ Alcotest.test_case "retry_config_defaults" `Quick test_retry_config_defaults
        ; Alcotest.test_case
            "truncate_price_to_precision"
            `Quick
            test_truncate_price_to_precision
        ] )
    ; ( "retry logic"
      , [ Alcotest.test_case
            "retry_with_backoff_logic"
            `Quick
            test_retry_with_backoff_logic
