...
let test_status_of_lighter_int_active () =
  Alcotest.(check bool) "2 -> New" true (Lighter.Types.status_of_lighter_int 2 = Types.New)
;;
...
let test_lighter_order_type_int () =
  ...
  Alcotest.(check int)
    "TakeProfit"
    3
    (Lighter.Types.lighter_order_type_int Types.TakeProfit);
  Alcotest.(check int)
    "StopLossLimit -> StopLoss"
    2
    (Lighter.Types.lighter_order_type_int Types.StopLossLimit);
  Alcotest.(check int)
    "TakeProfitLimit -> TakeProfit"
    3
    (Lighter.Types.lighter_order_type_int Types.TakeProfitLimit)
;;
...
let test_lighter_tif_int () =
  ...
  Alcotest.(check int)
    "GTC post_only"
    2
    (Lighter.Types.lighter_tif_int ~post_only:true Types.GTC)
;;
...
let test_side_of_is_ask () =
  Alcotest.(check bool)
    "true -> Sell"
    true
    (Lighter.Types.side_of_is_ask true = Types.Sell);
  Alcotest.(check bool)
    "false -> Buy"
    true
    (Lighter.Types.side_of_is_ask false = Types.Buy)
;;
...
let test_float_to_lighter_int () =
  ...
  Alcotest.(check int64)
    "price 2 dec"
    218183L
    (Lighter.Types.float_to_lighter_int ~decimals:2 2181.83);
  (* 1.5 with 4 decimals -> 15000 *)
  Alcotest.(check int64)
    "qty 4 dec"
    15000L
    (Lighter.Types.float_to_lighter_int ~decimals:4 1.5);
  (* 0.0 -> 0 *)
...
let test_parse_json_int () =
  Alcotest.(check int) "from string" 42 (Lighter.Types.parse_json_int (`String "42"));
  Alcotest.(check int) "from int" 7 (Lighter.Types.parse_json_int (`Int 7));
  Alcotest.(check int) "from float" 3 (Lighter.Types.parse_json_int (`Float 3.9));
  Alcotest.(check int) "bad string" 0 (Lighter.Types.parse_json_int (`String "abc"))
;;
...
let test_parse_json_int64 () =
  ...
  Alcotest.(check int64) "from int" 42L (Lighter.Types.parse_json_int64 (`Int 42));
  Alcotest.(check int64)
    "bad string"
    0L
    (Lighter.Types.parse_json_int64 (`String "notint"));
  Alcotest.(check int64) "null" 0L (Lighter.Types.parse_json_int64 `Null)
;;
  ...
  Alcotest.run
    ...
    [ ( "status_of_lighter_int"
      , [ Alcotest.test_case
            "InProgress (0)"
            `Quick
            test_status_of_lighter_int_in_progress
        ...
        ; Alcotest.test_case
            "CanceledByOwner (4)"
            `Quick
            test_status_of_lighter_int_canceled_by_owner
        ; Alcotest.test_case
            "CanceledByEngine (5)"
            `Quick
            test_status_of_lighter_int_canceled_by_engine
        ; Alcotest.test_case
            "CanceledByProtocol (6)"
            `Quick
            test_status_of_lighter_int_canceled_by_protocol
    ...
    ; ( "status_of_lighter_string"
      , [ Alcotest.test_case "Active" `Quick test_status_of_lighter_string_active
        ...
        ; Alcotest.test_case
            "Canceled variants"
            `Quick
            test_status_of_lighter_string_canceled_variants
    ...
    ; "tif", [ Alcotest.test_case "lighter_tif_int" `Quick test_lighter_tif_int ]
    ; ( "side"
    ...
    ; ( "int_float"
      , [ Alcotest.test_case "float_to_lighter_int" `Quick test_float_to_lighter_int
        ...
        ; Alcotest.test_case
            "price_increment_of_decimals"
            `Quick
            test_price_increment_of_decimals
        ; Alcotest.test_case
            "qty_increment_of_decimals"
            `Quick
            test_qty_increment_of_decimals
        ] )
    ; ( "rounding"
