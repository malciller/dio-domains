...
let test_fetch_fallback_sets_cache () =
  ...
  let cached =
    match F.get_cached () with
    | Some v -> v
    | None -> 0.0
  in
  let is_cached_valid =
    abs_float (cached -. fallback) < 0.0001 || abs_float (cached -. 20.0) < 0.0001
  in
...
let () =
  Alcotest.run
    ...
    ; ( "fetching"
      , [ Alcotest.test_case "fallback cache" `Quick test_fetch_fallback_sets_cache ] )
    ]
;;
