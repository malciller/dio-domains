...
let test_hl_order_type () =
  ...
  match hl_type2 with
  | Hyperliquid.Types.Limit { tif } ->
    Alcotest.(check string) "Market IOC" "Ioc" (Hyperliquid.Types.tif_to_string tif)
  | _ -> Alcotest.fail "Expected Limit"
;;
...
let test_format_number () =
  ...
  Alcotest.(check string)
    ...
    "1.234"
    (Hyperliquid.Actions.format_number 1.23400000);
  ...
  Alcotest.(check string)
    "small number"
    "0.0001"
    (Hyperliquid.Actions.format_number 0.0001)
;;

let test_to_hl_order_wire () =
  let wire =
    Hyperliquid.Actions.to_hl_order_wire
      ~qty:1.5
      ~symbol:"TEST"
      ~asset_index:5
...
let test_string_contains () =
  Alcotest.(check bool)
    "present"
    true
    (Hyperliquid.Actions.string_contains "hello world" "world");
  Alcotest.(check bool)
    "absent"
    false
    (Hyperliquid.Actions.string_contains "hello world" "xyz");
  Alcotest.(check bool)
    "empty substr"
    true
    (Hyperliquid.Actions.string_contains "hello" "");
  ...
  Alcotest.(check bool)
    "at start"
    true
    (Hyperliquid.Actions.string_contains "hello" "hel");
  ...
  Alcotest.(check bool)
    "longer substr"
    false
    (Hyperliquid.Actions.string_contains "hi" "hello")
;;
...
let test_is_retriable_error () =
  ...
  Alcotest.(check bool)
    "timeout"
    true
    (Hyperliquid.Actions.is_retriable_error "Connection timeout");
  Alcotest.(check bool)
    "connection"
    true
    (Hyperliquid.Actions.is_retriable_error "connection refused");
  Alcotest.(check bool)
    "network"
    true
    (Hyperliquid.Actions.is_retriable_error "network unreachable");
  Alcotest.(check bool)
    "reset"
    true
    (Hyperliquid.Actions.is_retriable_error "connection reset by peer");
  Alcotest.(check bool)
    "broken pipe"
    true
    (Hyperliquid.Actions.is_retriable_error "Broken pipe");
  (* Server errors *)
  Alcotest.(check bool)
    "500"
    true
    (Hyperliquid.Actions.is_retriable_error "HTTP Error 500");
  Alcotest.(check bool)
    "502"
    true
    (Hyperliquid.Actions.is_retriable_error "HTTP Error 502: Bad Gateway");
  Alcotest.(check bool)
    "503"
    true
    (Hyperliquid.Actions.is_retriable_error "503 Service Unavailable");
  Alcotest.(check bool)
    "504"
    true
    (Hyperliquid.Actions.is_retriable_error "504 Gateway Timeout");
  (* Rate limiting *)
  Alcotest.(check bool)
    "rate limit"
    true
    (Hyperliquid.Actions.is_retriable_error "Rate limit exceeded");
  Alcotest.(check bool)
    "too many requests"
    true
    (Hyperliquid.Actions.is_retriable_error "Too many requests");
  Alcotest.(check bool)
    "too many cumulative"
    true
    (Hyperliquid.Actions.is_retriable_error "Too many cumulative requests");
  (* Non-retriable *)
  Alcotest.(check bool)
    "rejected"
    false
    (Hyperliquid.Actions.is_retriable_error "Order rejected: insufficient balance");
  Alcotest.(check bool)
    "unknown symbol"
    false
    (Hyperliquid.Actions.is_retriable_error "Unknown symbol: XYZ");
...
let test_to_int64_opt () =
  Alcotest.(check (option int64))
    "Int"
    (Some 42L)
    (Hyperliquid.Actions.to_int64_opt (`Int 42));
  Alcotest.(check (option int64))
    "Intlit"
    (Some 123456789012345L)
    (Hyperliquid.Actions.to_int64_opt (`Intlit "123456789012345"));
  Alcotest.(check (option int64))
    "String"
    (Some 99L)
    (Hyperliquid.Actions.to_int64_opt (`String "99"));
  Alcotest.(check (option int64))
    "bad String"
    None
    (Hyperliquid.Actions.to_int64_opt (`String "abc"));
  Alcotest.(check (option int64)) "Null" None (Hyperliquid.Actions.to_int64_opt `Null);
  Alcotest.(check (option int64))
    "Float"
    None
    (Hyperliquid.Actions.to_int64_opt (`Float 1.5))
;;
