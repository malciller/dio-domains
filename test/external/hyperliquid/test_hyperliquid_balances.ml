let test_parse_json_float () =
  Alcotest.(check (float 0.000001))
    "from string"
    1.5
    (Hyperliquid.Balances.parse_json_float (`String "1.5"));
  Alcotest.(check (float 0.000001))
    "from float"
    2.5
    (Hyperliquid.Balances.parse_json_float (`Float 2.5));
  Alcotest.(check (float 0.000001))
    "from int"
    3.0
    (Hyperliquid.Balances.parse_json_float (`Int 3));
  Alcotest.(check (float 0.000001))
    "fallback"
    0.0
    (Hyperliquid.Balances.parse_json_float `Null)
;;
        ...
        , `Assoc
            [ ( "clearinghouseState"
              , `Assoc
                  ...
                  ; "marginSummary", `Assoc [ "accountValue", `String "1000.0" ]
                  ] )
            ; "userState", `Null
            ] )
      ]
...
let () =
  Alcotest.run
    ...
    [ "parsing", [ Alcotest.test_case "parse_json_float" `Quick test_parse_json_float ]
    ; ( "processing"
