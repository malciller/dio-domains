let test_round_price_to_tick () =
  (* Without a loaded pair cache, rounding falls back to 5 sig figs / 6 decimals. *)
  Alcotest.(check (float 0.000001))
    "round up"
    123.46
    (Hyperliquid.Instruments_feed.round_price_to_tick 123.456);
  Alcotest.(check (float 0.000001))
    "round down"
    123.45
    (Hyperliquid.Instruments_feed.round_price_to_tick 123.454);
  Alcotest.(check (float 0.000001))
    "small number"
    0.0012345
    (Hyperliquid.Instruments_feed.round_price_to_tick 0.00123449);
  (* Regression: the order_executor loop's identical-rounding edge case. *)
  ...
  (* Rounded stored/requested prices differ by less than the tick size. *)
  Alcotest.(check bool)
    "diff is correctly minimized"
    true
    (abs_float (rounded_requested -. rounded_stored) < tick);
  (* round_price_to_tick_for_symbol rounds to max(szDecimals - 1, MIN_DECIMALS).
     Idempotency: a second application does not change the result. *)
...
let test_round_price_for_symbol () =
  ...
  let rounded =
    Hyperliquid.Instruments_feed.round_price_to_tick_for_symbol "RNDTEST" 123.456789
  in
  Alcotest.(check bool) "rounded differs or equals" true (rounded > 0.0);
  (* Idempotency *)
  let r2 =
    Hyperliquid.Instruments_feed.round_price_to_tick_for_symbol "RNDTEST" rounded
  in
  ...
  let neg =
    Hyperliquid.Instruments_feed.round_price_to_tick_for_symbol "RNDTEST" (-5.0)
  in
