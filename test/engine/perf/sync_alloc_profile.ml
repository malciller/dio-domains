(* Allocation profile for [Jacobs_ladder.sync_open_orders].
   ...
   cross-module inlining, so reading a [float] back across the
   [Jacobs_ladder_sell_orders] module boundary boxes it (2 words/call) and the
   per-order index walks look quadratically allocating; release (-O3) inlines
   them unboxed, which is what production builds do.

   The production engine attributes a per-cycle minor-word budget to the sync
   stage ([alloc_sync_words]); this harness drives the *real* sync with a
   synthetic, prebuilt open-orders feed so the allocation can be attributed to
   the persisted-level rebuild vs the scan/reconcile, with no venue or I/O.
   The rebuild is now allocation-free in the steady no-change state, so the
   remaintain=T minus remaintain=F delta is the open/missing split only.

   The feed is precomputed so [iter_open_orders] allocates nothing per call
   (matching the venue iterators, which yield stored strings/literals). Each
   scenario is warmed up so commitment upserts and id tracking are steady-state,
   then measured over many calls. [remaintain_expired_sells] is the Alpaca flag
   that gates the persisted-ladder reconcile. *)
...
(* One scenario: [n_levels] persisted levels and [n_sells] feed sells. When the
   two differ the scenario is not aligned, so a rebuild/adoption may occur; the
   [aligned] rows below keep them equal to stay in the steady no-change regime. *)
let profile ~n_levels ~n_sells ~remaintain =
  ...
  let orders =
    List.init n_sells (fun i -> Printf.sprintf "s%d" i, 100.0 -. float i, 1.0, "sell", Some 1)
