...
let test_full_cycle () =
  ...
  Alcotest.(check string)
    "New short side is sell"
    "sell"
    (string_of_order_side new_short.side)
;;
...
let () =
  Alcotest.run
    ...
    ; ( "single_short"
      , [ Alcotest.test_case
            "Buy uses perp best bid for limit"
            `Quick
            test_hedge_buy_with_tob
        ; Alcotest.test_case
            "No TOB falls back to market"
            `Quick
            test_hedge_buy_no_tob_fallback
        ; Alcotest.test_case "Sell without hedge skips" `Quick test_sell_no_short_skips
        ; Alcotest.test_case
            "Sell closes full hedge at best ask"
            `Quick
            test_sell_closes_full_short
        ; Alcotest.test_case
            "Second buy skipped while hedge open"
            `Quick
            test_second_buy_skipped
        ; Alcotest.test_case
            "Full cycle: open, skip, close, reopen"
            `Quick
            test_full_cycle
        ] )
    ]
