...
let resolve
      ...
      ()
  =
  Dio_survival.Survival_tasks.resolve_tasks
    ~symbol
    ~exchange
    ~exchange_explicit
    ~trading
...
let pairs tasks =
  List.map
    (fun (t : Dio_survival.Survival_tasks.task) ->
       t.Dio_survival.Survival_tasks.symbol, t.Dio_survival.Survival_tasks.exchange)
    tasks
;;
...
let test_symbol_uses_config_exchange () =
  ...
  Alcotest.(check string)
    ...
    "hyperliquid"
    (List.hd tasks).Dio_survival.Survival_tasks.exchange
;;
...
let test_unknown_symbol_uses_defaults () =
  ...
  Alcotest.(check string)
    "symbol preserved"
    "FOO/BAR"
    t.Dio_survival.Survival_tasks.symbol;
  Alcotest.(check string)
    "exchange falls back to arg"
    "kraken"
    t.Dio_survival.Survival_tasks.exchange;
  Alcotest.(check string) "default qty" "1.0" t.Dio_survival.Survival_tasks.config.qty
;;
...
let test_calendar_kind () =
  Alcotest.(check bool)
    ...
    true
    (Dio_survival.Survival_tasks.calendar_kind_of_exchange "hyperliquid"
     = Dio_survival.Survival_types.Crypto);
  Alcotest.(check bool)
    "kraken is crypto"
    true
    (Dio_survival.Survival_tasks.calendar_kind_of_exchange "kraken"
     = Dio_survival.Survival_types.Crypto);
  Alcotest.(check bool)
    "alpaca is equity"
    true
    (Dio_survival.Survival_tasks.calendar_kind_of_exchange "alpaca"
     = Dio_survival.Survival_types.Equity);
  Alcotest.(check bool)
    "unknown defaults to crypto"
    true
    (Dio_survival.Survival_tasks.calendar_kind_of_exchange "binance"
     = Dio_survival.Survival_types.Crypto)
;;
...
let test_coin_of_symbol () =
  Alcotest.(check string)
    "btc/usdc"
    "BTC"
    (Dio_survival.Survival_fetch_hyperliquid.coin_of_symbol "BTC/USDC");
  Alcotest.(check string)
    "hype/usdc"
    "HYPE"
    (Dio_survival.Survival_fetch_hyperliquid.coin_of_symbol "HYPE/USDC");
  Alcotest.(check string)
    "eth/usd"
    "ETH"
    (Dio_survival.Survival_fetch_hyperliquid.coin_of_symbol "ETH/USD");
  Alcotest.(check string)
    "lowercase"
    "BTC"
    (Dio_survival.Survival_fetch_hyperliquid.coin_of_symbol "btc/usdc")
...
let test_parse_candles_sorts_and_dedups () =
  ...
  in
  let bars =
    Dio_survival.Survival_fetch_hyperliquid.parse_candles ~symbol:"BTC/USDC" json
  in
  Alcotest.(check int) "two bars after dedup" 2 (List.length bars);
  let dates =
    List.map
      (fun (b : Dio_survival.Survival_types.bar) -> b.Dio_survival.Survival_types.date)
      bars
  in
  ...
  Alcotest.(check (float 1e-9))
    "first close"
    100.5
    (List.hd bars).Dio_survival.Survival_types.close;
  Alcotest.(check (float 1e-9))
    "second close"
    102.5
    (List.nth bars 1).Dio_survival.Survival_types.close
;;
...
let test_parse_candles_bad_shape () =
  Alcotest.check_raises
    ...
    (Failure
       "Survival_fetch_hyperliquid.parse_candles: BTC/USDC expected array, got \
        {\"oops\":1}")
    (fun () ->
       ignore
         (Dio_survival.Survival_fetch_hyperliquid.parse_candles
            ~symbol:"BTC/USDC"
            (`Assoc [ "oops", `Int 1 ])))
;;
