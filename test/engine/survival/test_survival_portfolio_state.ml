let test_round_trip () =
  let path = Filename.temp_file "survival-positions" ".json" in
  let key =
    Dio_survival.Survival_topology.key ~venue:"hyperliquid" ~symbol:"BTC/USDC" ()
  in
  let values =
    [ { Dio_survival.Survival_portfolio_state.key; pool = 125.0; base = 0.5 } ]
  in
  (try
     Dio_survival.Survival_portfolio_state.save path values;
     match Dio_survival.Survival_portfolio_state.load path with
