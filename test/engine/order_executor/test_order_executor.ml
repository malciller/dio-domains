...
let test_generate_duplicate_key () =
  let key1 =
    Dio_engine.Order_executor.generate_duplicate_key "BTC/USD" "buy" 0.001 (Some 50000.0)
  in
...
let test_validate_order_request () =
  ...
  check
    ...
    "valid order request"
    (Ok ())
    (Dio_engine.Order_executor.validate_order_request valid_request);
  let empty_exchange = { valid_request with exchange = "" } in
  check
    bool
    "empty exchange invalid"
    true
    (Result.is_error (Dio_engine.Order_executor.validate_order_request empty_exchange));
  let zero_qty = { valid_request with quantity = 0.0 } in
  check
    bool
    "zero qty invalid"
    true
    (Result.is_error (Dio_engine.Order_executor.validate_order_request zero_qty));
  let missing_price = { valid_request with limit_price = None } in
  check
    bool
    "limit order without price invalid"
    true
    (Result.is_error (Dio_engine.Order_executor.validate_order_request missing_price))
;;
...
let test_validate_cancel_request () =
  ...
  check
    ...
    (Ok ())
    (Dio_engine.Order_executor.validate_cancel_request valid_cancel);
  ...
  check
    bool
    "no identifiers invalid"
    true
    (Result.is_error (Dio_engine.Order_executor.validate_cancel_request no_ids))
;;
...
let () =
  run
    "Order Executor"
    [ "duplicate_key", [ test_case "generate key" `Quick test_generate_duplicate_key ]
    ; ( "validation"
