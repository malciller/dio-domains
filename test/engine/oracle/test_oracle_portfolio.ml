(* Oracle_portfolio per-venue shared-budget tests.
...
let pbar ?(low = 97.5) ?(high = 98.5) () : Dio_oracle.Oracle_types.bar =
  { date = "d"; open_ = 100.0; high; low; close = low; volume = 1.0 }
;;

let run ?(transfers = []) ~positions () =
  Dio_oracle.Oracle_portfolio.simulate ~positions ~transfers ()
;;

let position ?(quote = "USD") ?(testnet = false) venue asset pool bars subgrids
  : Dio_oracle.Oracle_portfolio.position_input
  =
  { Dio_oracle.Oracle_portfolio.venue; asset; quote; testnet; pool; bars; subgrids }
;;

let find_o venue asset (r : Dio_oracle.Oracle_portfolio.result) =
  List.find
    (fun (o : Dio_oracle.Oracle_portfolio.position_outcome) ->
       o.venue = venue && o.asset = asset)
    r.positions
;;

let find_v venue quote (r : Dio_oracle.Oracle_portfolio.result) =
  List.find
    (fun (v : Dio_oracle.Oracle_portfolio.venue_outcome) ->
...
let test_transfer_rescues_position () =
  ...
  let a_key =
    Dio_oracle.Oracle_portfolio.{ venue = "kraken"; quote = "USD"; testnet = false }
  in
  let b_key =
    Dio_oracle.Oracle_portfolio.{ venue = "hype"; quote = "USDC"; testnet = false }
  in
  ...
  let with_transfer =
    ...
    in
    let transfers =
      [ { Dio_oracle.Oracle_portfolio.session = 1
        ; from = a_key
        ; to_ = b_key
        ; amount = 300.0
        }
...
let test_transfer_capped_at_pool () =
  ...
  let a_key =
    Dio_oracle.Oracle_portfolio.{ venue = "a"; quote = "USD"; testnet = false }
  in
  let b_key =
    Dio_oracle.Oracle_portfolio.{ venue = "b"; quote = "USD"; testnet = false }
  in
  let positions = [ position "a" "x" 100.0 [||] []; position "b" "y" 100.0 [||] [] ] in
  let transfers =
    [ { Dio_oracle.Oracle_portfolio.session = 0
      ; from = a_key
      ; to_ = b_key
      ; amount = 500.0
      }
...
let test_aligned_missing_bar_does_not_trade () =
  ...
  let positions : Dio_oracle.Oracle_portfolio.aligned_position_input list =
    [ { venue = "hype"
      ; asset = "HYPE"
      ; quote = "USDC"
      ; testnet = false
  ...
  in
  let result =
    Dio_oracle.Oracle_portfolio.simulate_aligned ~timeline ~positions ~transfers:[] ()
  in
