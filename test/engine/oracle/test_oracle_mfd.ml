...
let test_mfd_basic () =
  ...
  near 0.06 (Option.get (Dio_oracle.Oracle_mfd.mfd ~closes ~lows ~start:0 ~horizon:3));
  (* Half-open: MFD(0,1) = 1 - lows[1]/100 = 0.02 *)
  near 0.02 (Option.get (Dio_oracle.Oracle_mfd.mfd ~closes ~lows ~start:0 ~horizon:1))
;;
...
let test_surface () =
  ...
  let row =
    List.find
      (fun (r : Dio_oracle.Oracle_types.surface_row) -> r.drawdown_pct = 30.0)
      s.rows
  in
...
let test_stride_sampling () =
  ...
  Alcotest.(check int)
    "stride 1 windows"
    60
    (Dio_oracle.Oracle_mfd.n_starts ~closes ~lows ~horizon:30 ~warmup:10 ());
...
let test_mfd_truncated_window_excluded () =
  ...
  near 0.02 (Option.get (Dio_oracle.Oracle_mfd.mfd ~closes ~lows ~start:0 ~horizon:2));
  (* s=2, h=2 would need lows 3..4 - complete, so Some. *)
  Alcotest.(check bool)
    "complete tail window accepted"
    (Dio_oracle.Oracle_mfd.mfd ~closes ~lows ~start:2 ~horizon:2 <> None)
    true;
  ...
  Alcotest.(check bool)
    ...
    (Dio_oracle.Oracle_mfd.mfd ~closes ~lows ~start:3 ~horizon:2 = None)
    true;
  ...
  Alcotest.(check int)
    ...
    3
    (Dio_oracle.Oracle_mfd.n_starts ~closes ~lows ~horizon:2 ~warmup:0 ())
;;
...
let test_mfd_non_finite_guards () =
  ...
  Alcotest.(check bool)
    ...
    (Dio_oracle.Oracle_mfd.mfd ~closes ~lows ~start:1 ~horizon:1 = None)
    true;
  let closes = [| 100.; 100.; 100.; 100. |] in
  let lows = [| 100.; 98.; nan; 94. |] in
  Alcotest.(check bool)
    "NaN min low excluded"
    (Dio_oracle.Oracle_mfd.mfd ~closes ~lows ~start:0 ~horizon:2 = None)
    true;
  let closes = [| 100.; 100.; 100.; 100. |] in
...
let test_runway_cost_accumulates () =
  let c1 =
    Dio_oracle.Oracle_mfd.static_runway_cost
      ~qty:1.0
      ~grid_interval_pct:1.0
      ~fee:0.0
      ~start_price:100.0
...
let test_floor_aware_matches_closed_form_without_floor () =
  ...
  let c =
    Dio_oracle.Oracle_mfd.floor_aware_runway_cost
      ~qty:1.0
      ~grid_interval_pct:1.0
      ~fee:0.0
      ~start_price:100.0
