(* Oracle stats tests: percentiles, no-lookahead volatility, z, blend. *)

let near a b = Alcotest.(check (float 1e-6)) "approx" a b

let test_percentile () =
  let xs = [| 1.; 2.; 3.; 4.; 5. |] in
  near 3.0 (Dio_oracle.Oracle_stats.percentile xs 50.);
  near 1.0 (Dio_oracle.Oracle_stats.percentile xs 0.);
  near 5.0 (Dio_oracle.Oracle_stats.percentile xs 100.)
;;
...
let test_percentile_empty_raises () =
  ...
  (try
     ignore (Dio_oracle.Oracle_stats.percentile [||] 50.0);
     Alcotest.fail "percentile: expected Invalid_argument"
   with
   | Invalid_argument _ -> ());
  try
    ignore (Dio_oracle.Oracle_math.weighted_percentile [||] 50.0);
...
let test_percentile_ordering () =
  ...
  List.iter
    (fun p ->
       let v = Dio_oracle.Oracle_stats.percentile xs p in
       Alcotest.(check bool)
  ...
  List.iter
    (fun p ->
       let v = Dio_oracle.Oracle_math.weighted_percentile pairs p in
       Alcotest.(check bool)
...
let test_weighted_percentile_matches_unweighted () =
  ...
  List.iter
    (fun p ->
       let want = Dio_oracle.Oracle_stats.percentile xs p in
       let got = Dio_oracle.Oracle_math.weighted_percentile pairs p in
  ...
  (* Interpolation between the two brackets: P50 of {1, 10} with unit weights
     ...
     sample's anchor). *)
  near 5.5 (Dio_oracle.Oracle_math.weighted_percentile [| 1.0, 1.0; 10.0, 1.0 |] 50.0);
  near 1.0 (Dio_oracle.Oracle_math.weighted_percentile [| 1.0, 2.0; 10.0, 1.0 |] 50.0)
;;

let test_trailing_vol_constant () =
  let closes = Array.make 100 100.0 in
  near 0.0 (Option.get (Dio_oracle.Oracle_stats.trailing_vol ~closes ~s:80 ~w:60))
;;
...
let test_trailing_vol_no_lookahead () =
  ...
  Alcotest.(check bool)
    "no lookahead"
    (Dio_oracle.Oracle_stats.trailing_vol ~closes ~s:5 ~w:60 = None)
    true
;;
...
let test_trailing_vol_known () =
  ...
  let v = Option.get (Dio_oracle.Oracle_stats.trailing_vol ~closes ~s:60 ~w:60) in
...
let test_z_mfd () =
  ...
  let z =
    Option.get (Dio_oracle.Oracle_stats.z_mfd ~closes ~lows ~s:169 ~horizon:30 ~w:60)
  in
...
let z_samples ~closes ~lows =
  ...
  for s = 60 to Array.length closes - 1 do
    match Dio_oracle.Oracle_stats.z_mfd ~closes ~lows ~s ~horizon:30 ~w:60 with
    | Some z -> acc := z :: !acc
    | None -> ()
  done;
...
let test_asset_regime_zero_sigma () =
  (* Constant log-returns -> trailing vol is 0 up to FP noise, so every sigma
     is ~0.0 (excluded from the z-blend by Oracle_replay) and the start
...
let test_z_index_cdf () =
  ...
  let bars =
    Array.mapi
      ...
      c
  in
  let series : Dio_oracle.Oracle_types.series =
    { symbol = "T"; calendar_kind = Crypto; bars; gaps = [] }
  in
  let idx =
    Dio_oracle.Oracle_classes.z_index_of
  ...
  Alcotest.(check (float 1e-9))
    "cdf at 0"
    (Dio_oracle.Oracle_classes.z_cdf_of idx ~tau:0.0)
    (expected 0.0);
  let tau = 0.5 in
  Alcotest.(check (float 1e-9))
    "cdf at 0.5"
    (Dio_oracle.Oracle_classes.z_cdf_of idx ~tau)
    (expected tau);
  Alcotest.(check (float 1e-9))
    "cdf at +infinity"
    (Dio_oracle.Oracle_classes.z_cdf_of idx ~tau:Float.infinity)
    1.0
;;
