let bar date low : Dio_oracle.Oracle_types.bar =
...
let test_qualified_identity () =
  ...
  Alcotest.(check bool)
    ...
    false
    (Dio_oracle.Oracle_topology.equal_key hl kr)
;;
...
let test_parse_and_validate () =
  ...
  in
  match Dio_oracle.Oracle_topology.parse json with
  | Error error -> Alcotest.fail error
  | Ok definition ->
    (match Dio_oracle.Oracle_topology.validate definition with
     | Ok () -> ()
...
let test_align_without_forward_fill () =
  ...
  let aligned = Dio_oracle.Oracle_topology.align_series timeline series in
