...
let wait_until_ready () =
  ...
  then Lwt.return_true
    ...
    Lwt_condition.wait ready_condition >>= fun () -> Lwt.return_true
;;
...
let publish_balance_update storage_key balance =
  ...
  BalanceUpdateEventBus.publish
    balance_update_event_bus
    { asset = storage_key
...
let process_asset_balances json =
  ...
  let extract_float_opt key =
    ...
    if v <> `Null
    then (
      try Some (Lighter_types.parse_json_float v) with
      | _ -> None)
    else None
  in
  ...
  then (
    ...
    Logging.debug_f
      ~section
      "Unified USDC updated from account_all: %.6f (col:%.2f avail:%.2f)"
      unified_usdc
      acct_collateral
      acct_available);
  let assets =
    try member "assets" account_data |> to_assoc with
    | _ -> []
  in
  if assets = []
  then
    Logging.debug_f
      ~section
      "Balance update has no assets field (type=%s, content=%s)"
...
let process_user_stats json =
  ...
  let stats =
    ...
    then s
      ...
      if us <> `Null
      then member "stats" us
      else (
        let d = member "data" json in
        if d <> `Null then member "stats" d else `Null))
  in
  if stats <> `Null
  then (
    let extract_float_opt key =
    ...
    then (
      ...
      Logging.debug_f
        ~section
        "USDC updated from user_stats: %.6f (col:%.2f mb:%.2f wb:%.2f av:%.2f)"
        usdc_balance
        c
        m
        w
      ...
      Logging.debug_f
        ~section
        "user_stats zero balance (existing=%.2f, col:%.2f mb:%.2f wb:%.2f av:%.2f)"
        existing
        c
        m
        w
...
let process_market_data json =
  ...
  | "subscribed/account_all_assets" ->
    (try process_asset_balances json with
     | exn ->
       Logging.error_f
         ~section
         "Failed to process asset balance update: %s"
         (Printexc.to_string exn))
  | "update/user_stats" | "snapshot/user_stats" | "subscribed/user_stats" ->
    (try process_user_stats json with
     | exn ->
       Logging.error_f
         ~section
         "Failed to process user_stats update: %s"
         (Printexc.to_string exn))
  | _ -> ()
;;
...
let initialize assets =
  Logging.info_f
    ~section
    "Initializing Lighter balances feed for %d assets"
    (List.length assets);
