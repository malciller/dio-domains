...
(** Keeps the first [max_depth] levels; input is already sorted best-first. *)
...
let process_orderbook_snapshot ~market_index json =
  ...
  in
  try
    (* Lighter nests the payload under [order_book], not [data]. *)
    ...
    then (
      ...
      then (
        ...
        Logging.info_f
          ~section
          "Orderbook snapshot for %s: order_book is null, top-level keys: [%s]"
          symbol
          (String.concat ", " keys)))
    else (
      (* Log top-level keys once on the first valid snapshot. *)
      if not (Atomic.exchange first_snapshot_logged true)
      then (
        let keys =
          try List.map fst (to_assoc ob_data) with
          | _ -> []
        in
        Logging.info_f
          ~section
      ...
      let parse_level_opt level =
        ...
        | _ -> None
      in
      let parse_levels levels_json = List.filter_map parse_level_opt levels_json in
      ...
      | Some store ->
        ...
        store.local_bids
        <- truncate_to_depth (List.sort (fun (a, _) (b, _) -> compare b a) bids);
        store.local_asks
        <- truncate_to_depth (List.sort (fun (a, _) (b, _) -> compare a b) asks);
        flush_to_ring store symbol;
        Mutex.unlock store.ob_mutex;
        Logging.debug_f
          ~section
          "Orderbook snapshot for %s: bids=%d asks=%d"
          symbol
          (List.length bids)
          (List.length asks))
  with
  | exn ->
    Logging.warn_f
      ~section
      "Failed to parse orderbook snapshot for market %d: %s"
      market_index
  ...
  try
    ...
    then ()
      ...
      | Some store ->
        ...
        Logging.debug_f
          ~section
          "Orderbook delta for %s: %d bid updates, %d ask updates"
          symbol
          (List.length bids_json)
          (List.length asks_json))
  with
  | exn ->
    Logging.warn_f
      ~section
      "Failed to parse orderbook update for market %d: %s"
      market_index
      (Printexc.to_string exn)
;;
...
let initialize symbols =
  Logging.info_f
    ~section
    "Initializing Lighter orderbook feed for %d symbols"
    (List.length symbols);
  List.iter
    (fun symbol ->
