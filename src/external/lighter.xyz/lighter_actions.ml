(** Order actions for Lighter: place/cancel/modify via signed transactions. Dispatch uses
    REST [POST /api/v1/sendTx] via the configured LIGHTER_PROXY; signing happens in
    [lighter_signer.ml] over FFI. The WS [jsonapi/sendtx] path in [lighter_ws.ml] is
    unused. *)
...
(** Client order index counter. Lighter requires a unique client order index per placement
    across all markets; seeded from epoch millis so restarts do not collide with
    previously issued indices. *)
...
(** Submits a signed transaction over REST as multipart form data, matching the Python
    SDK: fields [tx_type] (integer) and [tx_info] (JSON string produced by the Go signer). *)
    ...
    (fun () ->
      ...
      then (
        ...
        Logging.error_f
          ...
          resp_str;
        (* Auto-recovery for nonce desync: operations sent during WS outages consume
           nonces locally but may never confirm on chain, diverging the local counter from
           the exchange. Re-fetch the authoritative nonce so later orders do not loop on
           invalid-nonce rejections. *)
...
(** Places a new order: converts params to Lighter's integer formats, signs the
    create-order tx, and pre-registers the order in the local execution feed keyed by
    client order index until the exchange assigns its own order id. *)
let place_order
  ~symbol
  ~is_buy
  ~qty
  ...
  | Some market_index ->
    (match Lighter_instruments_feed.lookup_info symbol with
     ...
     | Some info ->
       ...
       (* Lighter has no native GTC; expiry follows GTT semantics. Passing -1 tells the Go
          signer to compute the default order expiry, matching the Python SDK. *)
       ...
       (match result with
        | Ok _ ->
          ...
          Logging.info_f
            ...
            price
            market_index;
          (* Pre-register the order under the client index; the WS feed later confirms the
             exchange-assigned order index. *)
          let side =
...
(** Modifies an order's qty/price: converts to Lighter integers, signs, and submits while
    keeping the original order id. *)
...
(** Cancels all open orders in one market: resolves the market index and submits a signed
    cancel-all tx. *)
