(** Lighter-specific type definitions and conversion helpers. Maps between Lighter L2's
    integer-based protocol and the exchange-agnostic [Exchange_intf.Types]. *)
...
(** Lighter order status integer codes to [Types.order_status]. Reference: Lighter API
    documentation. *)
...
(** Map [Types.order_type] to Lighter's integer codes. 0=Limit, 1=Market, 2=StopLoss,
    3=TakeProfit *)
...
(** Map [Types.time_in_force] to Lighter's integer codes. 0=IOC, 1=GoodTillTime,
    2=PostOnly *)
...
(** Map [Types.order_side] to Lighter's is_ask boolean. Sell (ask) = true, Buy (bid) =
    false. *)
...
(** Convert a float price/size to Lighter's integer representation using the specified
    decimal precision. E.g., price 2181.83 with 2 decimals → 218183 *)
...
(** Convert Lighter's integer representation back to a float. E.g., 218183 with 2 decimals
    → 2181.83 *)
...
let[@inline always] price_increment_of_decimals decimals = 10.0 ** -.float_of_int decimals

(** Quantity increment derived from [supported_size_decimals]. *)
let[@inline always] qty_increment_of_decimals decimals = 10.0 ** -.float_of_int decimals

(** Round price to the exchange's valid precision for a given market. Rounds to nearest
    tick using [supported_price_decimals]. *)
