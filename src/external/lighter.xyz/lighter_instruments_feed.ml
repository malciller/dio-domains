(** Lighter instruments feed. Fetches market metadata from the
    [GET /api/v1/orderBookDetails] REST endpoint at startup, builds a bidirectional
    mapping between textual symbols and numeric market indices, and validates that all
    user configured symbols exist on the Lighter exchange. Also provides price and
    quantity precision rounding helpers for outbound order payloads. *)
...
let wait_until_ready () =
  if Atomic.get is_ready then Lwt.return_unit else Lwt_condition.wait ready_condition
;;
...
(** Fetch and parse market metadata from the [/api/v1/orderBookDetails] REST endpoint.
    Populates the internal [pair_cache] mapping strings to market info, and
    [index_to_symbol] mapping integer IDs to string symbols. Validates that all symbols in
    the [required_symbols] list exist; throws an exception to halt startup if validation
    fails. *)
            ...
            List.iter
              (fun market ->
                try
                  ...
                  (* Lighter returns bare symbols for perpetual futures ("ETH") and
                     dash-separated symbols for spot markets ("ETH-USDC"). Perpetual
                     symbols stay raw to avoid collision with spot ETH/USDC, registered
                     separately at index 2048. *)
                  let symbol = raw_symbol in
                  let supported_size_decimals =
                    member "supported_size_decimals" market |> to_int
                  in
                  let supported_price_decimals =
                    member "supported_price_decimals" market |> to_int
                  in
                  let min_base_amount =
                    Lighter_types.parse_json_float (member "min_base_amount" market)
                  in
                  let min_quote_amount =
                    Lighter_types.parse_json_float (member "min_quote_amount" market)
                  in
                  let taker_fee =
                    try Lighter_types.parse_json_float (member "taker_fee" market) with
                    | _ -> 0.0
                  in
                  let maker_fee =
                    try Lighter_types.parse_json_float (member "maker_fee" market) with
                    | _ -> 0.0
                  in
                  ...
                  Logging.debug_f
                    ~section
                    "Registered market: %s (raw=%s, index=%d, price_dec=%d, size_dec=%d)"
                    symbol
                    raw_symbol
                    market_index
                    supported_price_decimals
              ...
              markets);
        (* Spot markets are absent from orderBookDetails (perps only) but appear in the
           explorer's /api/markets with indices >= 2048. Register any market missing from
           pair_cache, copying precision/mins from the base perp contract (ETH/USDC
           derives from ETH, LIT/USDC from LIT). *)
        let%lwt () =
          Lwt.catch
            (fun () ->
              ...
              let%lwt _resp, body =
                Cohttp_lwt_unix.Client.get (Uri.of_string markets_url)
              in
              let%lwt body_str = Cohttp_lwt.Body.to_string body in
              let markets_json = Yojson.Safe.from_string body_str in
              let all_markets =
                match markets_json with
                | `List items -> items
                | _ -> []
              in
              Mutex.lock cache_mutex;
                  ...
                  List.iter
                    (fun entry ->
                      try
                        ...
                        (* Register only symbols missing from pair_cache, i.e. spot
                           markets. *)
                        ...
                        then (
                          ...
                          let size_dec, price_dec, min_base, min_quote =
                            match perp_info with
                            | Some pi ->
                              ( pi.supported_size_decimals
                              , pi.supported_price_decimals
                              , pi.min_base_amount
                              , pi.min_quote_amount )
                            | None -> 4, 2, 0.005, 10.0
                          in
                          ...
                          Logging.info_f
                            ~section
                            "Registered spot market: %s (index=%d, precision from %s \
                             perp)"
                            sym
                            mi
                            perp_symbol)
                        else if (* Backfill index_to_symbol for markets registered
                                   earlier. *)
              ...
              Logging.info_f
                ~section
                "Spot market discovery complete: %d total markets in /api/markets"
                (List.length all_markets);
              Lwt.return_unit)
            (fun exn ->
              Logging.warn_f
                ~section
                "Failed to fetch spot markets from explorer: %s (spot symbols may be \
                 unavailable)"
                (Printexc.to_string exn);
              Lwt.return_unit)
        in
        ...
        let missing =
          List.filter
            (fun sym ->
              ...
              Fun.protect
                ~finally:(fun () -> Mutex.unlock cache_mutex)
                (fun () -> not (Hashtbl.mem pair_cache sym)))
            required_symbols
        in
        if missing <> []
        then (
          ...
          Logging.error_f
            ~section
            "FATAL: Required symbols not found on Lighter: [%s]"
            missing_str;
          Logging.error_f
            ~section
            "Available symbols: [%s]"
            (Mutex.lock cache_mutex;
             let syms =
               Fun.protect
                 ~finally:(fun () -> Mutex.unlock cache_mutex)
                 (fun () -> Hashtbl.fold (fun sym _ acc -> sym :: acc) pair_cache [])
             in
             String.concat ", " syms);
          failwith (Printf.sprintf "Lighter: required symbols not found: %s" missing_str));
        Logging.info_f
          ~section
          "Initialized Lighter instruments feed with %d markets (%d required symbols \
           validated)"
          (List.length markets)
          (List.length required_symbols);
        notify_ready ();
        Lwt.return_unit)
      (fun exn ->
        Logging.error_f
          ~section
          "Failed to fetch orderBookDetails: %s"
          (Printexc.to_string exn);
        notify_ready ();
        Lwt.return_unit)
  in
...
let get_price_increment symbol =
  match lookup_info symbol with
  | Some info ->
    Some (Lighter_types.price_increment_of_decimals info.supported_price_decimals)
  | None -> None
;;
...
let get_qty_increment symbol =
  match lookup_info symbol with
  | Some info ->
    Some (Lighter_types.qty_increment_of_decimals info.supported_size_decimals)
  | None -> None
;;
...
let round_price_for_symbol symbol price =
  match lookup_info symbol with
  | Some info ->
    Lighter_types.round_price ~price_decimals:info.supported_price_decimals price
  | None -> price
;;
...
let initialize symbols =
  ...
  Logging.info_f
    ~section
    "Initialized Lighter instruments feed with %d mock symbols"
    (List.length symbols)
;;
