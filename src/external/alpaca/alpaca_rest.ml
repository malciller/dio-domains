...
let parse_iso_timestamp s =
  ...
  then 0.0
    ...
    try
      ...
      let sub_sec =
        if String.length s > 20 && s.[19] = '.'
        then (
          let end_idx =
            try String.index_from s 20 'Z' with
            | _ -> String.length s
          in
          let frac_str = String.sub s 19 (end_idx - 19) in
          try float_of_string frac_str with
          | _ -> 0.0)
        else 0.0
      in
  ...
  let fetch feed_name =
    ...
    then (
      try
        ...
        if valid_quote && lq_ts >= lt_ts && lq_ts >= mb_ts
        then Lwt.return (Ok (bp, bs, ap, as_val))
        else if valid_trade && lt_ts >= lq_ts && lt_ts >= mb_ts
        then Lwt.return (Ok (tp, ts, tp, ts))
        else if valid_bar && mb_ts >= lq_ts && mb_ts >= lt_ts
        then Lwt.return (Ok (mp, ms, mp, ms))
        else if valid_quote
        then Lwt.return (Ok (bp, bs, ap, as_val))
        else if valid_trade
        then Lwt.return (Ok (tp, ts, tp, ts))
        ...
        then Lwt.return (Ok (bp, bs, bp, bs))
        else if ap > 0.0
        then Lwt.return (Ok (ap, as_val, ap, as_val))
        else
          Lwt.return
            (Error
               (Printf.sprintf
                  "No valid quote/trade/bar price for %s in %s snapshot"
                  symbol
                  feed_name))
      with
      | exn ->
        let err =
          Printf.sprintf
            "Failed to parse %s snapshot response: %s"
            feed_name
            (Printexc.to_string exn)
        in
        Lwt.return (Error err))
    ...
    (fun () ->
       ...
       then fetch primary_feed
         ...
         | Error _ ->
           ...
           then fetch "overnight"
           else Lwt.return (Error "Overnight snapshot failed"))
    (fun exn ->
       let err =
         Printf.sprintf "Get snapshot HTTP exception: %s" (Printexc.to_string exn)
       in
       Logging.error_f ~section "%s" err;
       Lwt.return (Error err))
;;
