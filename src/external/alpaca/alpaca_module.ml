...
module Alpaca_impl = struct
  ...
  let amend_order
    ...
    let fallback_cancel_replace sym q lp side =
      ...
      | Error e -> Lwt.return (Error (Printf.sprintf "Fallback cancel_order failed: %s" e))
    in
    let find_existing () =
      match symbol with
    ...
    in
    if is_fractional then begin
      match find_existing (), qty, limit_price with
      | Some existing, Some q, Some lp ->
          fallback_cancel_replace existing.symbol q lp (match existing.side with Buy -> Alpaca_types.Buy | Sell -> Sell)
      | _ -> perform_rest_amend ()
    end else begin
      perform_rest_amend () >>= function
  ...
  let cancel_orders
    ...
    | Some ids ->
        Lwt_list.map_s Alpaca_rest.cancel_order ids >|= fun results ->
        let succs = List.filter_map (function Ok r -> Some r | Error _ -> None) results |> List.concat in
        let mapped = List.map (fun (r : Alpaca_types.cancel_order_result) ->
          { Types.order_id = r.order_id; cl_ord_id = r.cl_ord_id }
        ) succs in
        if mapped <> [] then Ok mapped
        else Error "Failed to cancel orders"
    | None -> Lwt.return (Error "No order_ids provided for Alpaca cancellation")

  let get_top_of_book ~symbol =
    Alpaca_orderbook.get_best_bid_ask symbol

  let get_top_of_book_fast ~symbol =
    Alpaca_orderbook.get_best_bid_ask_fast symbol

  let get_tradeable_balance ~asset =
    Alpaca_balances.get_balance asset

  let get_tradeable_balance_fast ~asset =
    (fun () -> Alpaca_balances.get_balance asset)

  let get_total_balance ~asset =
    Alpaca_balances.get_total_balance asset

  let get_all_balances () =
    Alpaca_balances.get_all_balances ()
  ...
  let get_open_orders ~symbol =
    ...
    List.map (fun (o : Alpaca_executions.open_order_internal) ->
      { Types.
        order_id = o.order_id;
        symbol = o.symbol;
        side = o.side;
  ...
  let get_all_orders_for_asset ~asset =
    ...
    let matching = List.filter (fun sym -> sym = asset || String.starts_with ~prefix:(asset ^ "/") sym) all_symbols in
    List.concat_map (fun symbol -> get_open_orders ~symbol) matching

  let subscribe_orderbook ~symbols =
    Alpaca_orderbook.subscribe_symbols symbols

  let get_orderbook_position ~symbol =
    Alpaca_orderbook.get_current_position symbol
  ...
  let iter_top_of_book_events ~symbol ~start_pos f =
    Alpaca_orderbook.iter_orderbook_events symbol start_pos (fun ob ->
      if Array.length ob.bids > 0 && Array.length ob.asks > 0 then
        f (fst ob.bids.(0)) (snd ob.bids.(0)) (fst ob.asks.(0)) (snd ob.asks.(0))
    )

  let get_execution_feed_position ~symbol =
    Alpaca_executions.get_current_position symbol
  ...
  let has_execution_data ~symbol =
    Alpaca_executions.has_execution_data symbol

  let has_execution_data_fast ~symbol =
    Alpaca_executions.has_execution_data_fast symbol

  let read_execution_events ~symbol ~start_pos =
    let events = Alpaca_executions.read_execution_events symbol start_pos in
    List.map (fun (e : Alpaca_executions.execution_event_internal) ->
      { Types.
        order_id = e.order_id;
  ...
  let iter_execution_events ~symbol ~start_pos f =
    Alpaca_executions.iter_execution_events symbol start_pos (fun (e : Alpaca_executions.execution_event_internal) ->
      f { Types.
        order_id = e.order_id;
        order_status = e.order_status;
        limit_price = e.limit_price;
  ...
  let fold_open_orders ~symbol ~init ~f =
    Alpaca_executions.fold_open_orders symbol ~init ~f:(fun acc (o : Alpaca_executions.open_order_internal) ->
      f acc { Types.
        order_id = o.order_id;
        symbol = o.symbol;
        side = o.side;
  ...
  let iter_open_orders_fast ~symbol f =
    Alpaca_executions.fold_open_orders symbol ~init:() ~f:(fun () (o : Alpaca_executions.open_order_internal) ->
      let limit_price = match o.limit_price with Some p -> p | None -> 0.0 in
      let side_str = match o.side with Types.Buy -> "buy" | Sell -> "sell" in
      f o.order_id limit_price o.remaining_qty side_str o.user_ref
    )

  let get_price_increment ~symbol:_ = Some 0.01

  let get_qty_increment ~symbol:_ = Some 0.000000001 (* Native Alpaca 9-decimal fractional precision *)
...
let () =
  Exchange.Registry.register (module Alpaca_impl)
