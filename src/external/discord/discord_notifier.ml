...
let format_fill_field (fill : Concurrency.Fill_event_bus.fill_event) =
  ...
  let value =
    Printf.sprintf
      ...
      fill.fee
  in
  `Assoc [ "name", `String name; "value", `String value; "inline", `Bool true ]
;;
...
let build_webhook_payload (fills : Concurrency.Fill_event_bus.fill_event list) =
  ...
  | [ fill ] ->
    ...
    let embed =
      `Assoc
        [ ( "title"
          , `String
              (Printf.sprintf "%s - %s @ $%.4f" fill.symbol side_label fill.fill_price) )
        ; "color", `Int color
        ; ( "fields"
          , `List
              [ `Assoc
                  [ "name", `String "Exchange"
                  ; "value", `String (Printf.sprintf "`%s`" fill.venue)
                  ; "inline", `Bool true
                  ]
              ; `Assoc
                  [ "name", `String "Pair"
                  ; "value", `String (Printf.sprintf "`%s`" fill.symbol)
                  ; "inline", `Bool true
                  ]
              ; `Assoc
                  [ "name", `String "Side"
                  ; "value", `String side_label
                  ; "inline", `Bool true
                  ]
              ; `Assoc
                  [ "name", `String "Amount"
                  ; "value", `String (Printf.sprintf "`%.8g`" fill.amount)
                  ; "inline", `Bool true
                  ]
              ; `Assoc
                  [ "name", `String "Fill Price"
                  ; "value", `String (Printf.sprintf "`$%.4f`" fill.fill_price)
                  ; "inline", `Bool true
                  ]
              ; `Assoc
                  [ "name", `String "Value"
                  ; "value", `String (Printf.sprintf "`$%.2f`" fill.value)
                  ; "inline", `Bool true
                  ]
              ] )
        ; ( "timestamp"
          , `String
              (let t = Unix.gmtime fill.timestamp in
               Printf.sprintf
  ...
  | fills ->
    ...
    let color = 0x3498DB in
    (* Blue for batched *)
    let venues =
      List.sort_uniq
        String.compare
        (List.map (fun (f : Concurrency.Fill_event_bus.fill_event) -> f.venue) fills)
    in
  ...
  let rec attempt n =
    ...
    then (
      ...
      | Error "rate_limited" ->
        ...
        attempt (n + 1)
      | Error _ -> Lwt_unix.sleep 5.0 >>= fun () -> attempt (n + 1)
  in
  attempt 1
;;
