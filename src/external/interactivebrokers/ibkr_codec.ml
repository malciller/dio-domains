(** TWS API protocol codec.
    ...
    Encodes outbound messages as length-prefixed, null-delimited field sequences and
    decodes inbound messages from the same format. Pure functions: no IO, no state.
    ...
    Each field is a UTF-8 string terminated by a null byte. The 4-byte big-endian length
    prefix covers everything after itself. *)

(* ---- Encoding (outbound) ---- *)

(** [encode_fields fields] joins [fields] with null delimiters and prepends a 4-byte
    big-endian length prefix. Returns a [Bytes.t] ready to write to the socket. *)
...
(** [encode_handshake min_ver max_ver] builds the initial API handshake message:
    ["API\x00"] followed by a length-prefixed version range string. *)
...
(** [decode_fields payload] splits a null-delimited payload into a list of string fields.
    Drops trailing empty fields produced by the terminating null. *)
...
(** Cursor-based field reader. Avoids allocation on each read by operating on the raw
    field list via pattern matching. *)

(** [read_string fields] pops the next field as a string. Returns
    [(value, remaining_fields)]. *)
let read_string = function
  | s :: rest -> s, rest
  | [] -> "", []
;;

(** [read_int fields] pops the next field as an int. Returns [(value, remaining_fields)].
...
(** [read_float fields] pops the next field as a float. Handles TWS's
    "1.7976931348623157E308" (Double.MAX_VALUE) as 0.0. Returns
    [(value, remaining_fields)]. *)
let read_float = function
  | s :: rest ->
    let v =
      if s = "" || s = "1.7976931348623157E308"
      then 0.0
      else (
...
(** [read_bool fields] pops the next field as a bool. "1" or "true" -> true, anything else
    -> false. Returns [(value, remaining_fields)]. *)
...
(** [read_int_max fields] pops the next field as an int, treating empty string or
    MAX_VALUE as [max_int]. *)
...
(** [encode_contract c] serializes a contract into the field sequence expected by TWS for
    reqContractDetails / reqMktData / placeOrder. The exact field order is dictated by the
    TWS API protocol version. *)
...
(** [encode_contract_short c] serializes only the minimal contract fields needed for
    reqMktData (no includeExpired/comboLegs). *)
...
(** [encode_order o] serializes the initial order fields for the placeOrder message. These
    are the first set of fields after the contract + secIdType + secId. *)
...
(** [encode_order_tail ()] emits the remaining ~60 fields required by placeOrder. All set
    to empty/default values for simple STK limit/market orders. Field order matches
    ib_insync client.py placeOrder exactly. *)
let encode_order_tail () : string list =
  ...
  ; (* percentOffset *)
    "0"
  ; (* eTradeOnly - deprecated but still sent *)
    "0"
  ; (* firmQuoteOnly - deprecated but still sent *)
    ""
