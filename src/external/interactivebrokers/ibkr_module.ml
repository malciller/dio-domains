(** IBKR adapter implementing [Exchange_intf.S].
    ...
    Translates between TWS-specific data structures and the canonical Dio types. Order
    operations delegate to [Ibkr_actions]; market data reads hit per-symbol in-memory feed
    caches.
...
(** Runtime configuration from environment variables. The [testnet] flag (per-symbol in
    the config manifest) is applied by the supervisor before connecting, overriding
    [trading_mode], [is_paper], and [gateway_port]. *)
module Config = struct
  ...
  (** Applies the [testnet] flag: true forces paper on port 4002, false forces live on
      port 4001. The port is overridden only when IBKR_GATEWAY_PORT was not set in the
      environment. *)
...
module Ibkr_impl = struct
  ...
  let get_tradeable_balance_fast ~asset () = Ibkr_balances.get_balance ~asset

  (* IBKR's reported figure is already hold-netted. *)
  ...
  (** [true] once the initial execution snapshot has landed for [symbol]. *)
  ...
  let drain_open_order_changes ~symbol =
    Ibkr_executions_feed.drain_open_order_changes ~symbol
  ;;
  ...
  let get_fees ~symbol:_ =
    (* IBKR commissions are tiered, not per-instrument. Returning None makes downstream
       consumers treat the fee as 0.0. *)
    None, None
  ;;

  (* IBKR ingestion is REST-poll based; it exposes no raw frame stream to decode, so the
     uniform entry point is a no-op. *)
