  ...
  let fetch =
    Lwt.catch (fun () ->
      ...
      let missing = List.filter (fun sym ->
        Mutex.lock cache_mutex;
        let found = Hashtbl.mem pair_cache sym in
        Mutex.unlock cache_mutex;
        not found
      ) required_symbols in
      ...
      if missing <> [] then begin
        ...
        Logging.error_f ~section "Available symbols: [%s]"
          (Mutex.lock cache_mutex;
           let syms = Hashtbl.fold (fun sym _ acc -> sym :: acc) pair_cache [] in
           Mutex.unlock cache_mutex;
           String.concat ", " syms);
...
let lookup_info symbol =
  Mutex.lock cache_mutex;
  let result = Hashtbl.find_opt pair_cache symbol in
  Mutex.unlock cache_mutex;
  result
...
let get_symbol ~market_index =
  Mutex.lock cache_mutex;
  let result = Hashtbl.find_opt index_to_symbol market_index in
  Mutex.unlock cache_mutex;
  result
