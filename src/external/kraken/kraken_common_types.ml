...
let get_conduit_ctx () =
  ...
  | CamlinternalLazy.Undefined ->
    Logging.error
      ~section
      "Conduit context was accessed before initialization - this should not happen";
    raise
      (Failure
         "Conduit context not initialized - ensure main.ml initializes it before domain \
          spawning")
  | exn ->
...
let normalize_base64_secret (secret : string) : string =
  ...
  let padding =
    (base64_block_size - (String.length sanitized mod base64_block_size))
    mod base64_block_size
  in
...
let decode_secret_base64 (secret : string) : string =
  ...
  | Ok decoded -> decoded
  | Error (`Msg msg) ->
    failwith (Printf.sprintf "Invalid Kraken API secret (base64 decode failed): %s" msg)
;;
...
let sign ~secret ~path ~body ~nonce =
  ...
  let hmac =
    Digestif.SHA512.(
      hmac_string ~key:(decode_secret_base64 secret) (path ^ sha256) |> to_raw_string)
  in
