...
module Kraken_impl = struct
  ...
  let amend_order
    ...
    | None ->
      Logging.warn_f
        ~section
        "amend_order: order %s not in local cache%s — returning error for clean recovery"
        order_id
  ...
  let fold_open_orders ~symbol ~init ~f =
    Kraken_executions_feed.fold_open_orders
      symbol
      ~init
      ~f:(fun acc (o : Kraken_executions_feed.open_order) ->
        f
          acc
          { Types.order_id = o.order_id
          ; symbol = o.symbol
  ...
  let iter_open_orders_fast ~symbol f =
    Kraken_executions_feed.fold_open_orders
      symbol
      ~init:()
      ~f:(fun () (o : Kraken_executions_feed.open_order) ->
        let limit_price =
        ...
        let side_str =
          match o.side with
          | Kraken_executions_feed.Buy -> "buy"
          | Sell -> "sell"
        in
