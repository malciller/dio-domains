...
(** Canonical, slash-free pair name (legacy REST [descr.pair] form) so a REST order can be
    mapped back to the configured symbol. Mirrors the instruments feed's canonicalizer
    (XXBT/XBT -> BTC, ZUSD -> USD, ...). *)
...
(** Maps a REST [descr.pair] to the configured strategy symbol (e.g. "XMRUSD" ->
    "XMR/USD"); falls back to the raw pair when unmatched. *)
...
let get_open_orders ~symbols () : Kraken_executions_feed.execution_event list Lwt.t =
  Lwt.catch
    (fun () ->
      ...
      let encoded_body =
        Uri.encoded_of_query [ "nonce", [ nonce ]; "trades", [ "true" ] ]
      in
      let signature =
      ...
      Logging.debug
        ~section
        "Fetching REST OpenOrders payload to bypass WS 50-order snapshot limit";
      Client.post
        ~headers
      ...
      then (
        ...
        | _ ->
            ...
            List.filter_map
              (fun (order_id, order_json) ->
                try
                  ...
                  let vol_exec =
                    member "vol_exec" order_json |> to_string |> float_of_string
                  in
                  ...
                  let avg_price =
                    member "price" order_json |> to_string |> float_of_string
                  in
                  let userref_opt =
                    match member "userref" order_json with
                    | `Int i -> Some i
                  ...
                  in
                  let cl_ord_id_opt =
                    match member "clordid" order_json with
                    | `String s -> Some s
                    | _ -> None
                  in
                  Some
                ...
                | e ->
                  Logging.warn_f
                    ~section
                    "Failed to parse open order %s: %s"
                    order_id
          ...
          Logging.info_f
            ~section
            "Retrieved %d total open orders via REST (Bypassed WS 50-order cap)"
