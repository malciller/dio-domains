...
(** Parses a Kraken API status string into a [pair_status]. *)
...
(** Serializes a [pair_status] into its Kraken API string form. *)
...
(** Returns [true] only when the status is [Online]. *)
let is_tradeable = function
  | Online -> true
  | _ -> false
;;
...
let process_instrument_data json =
  ...
  | exn ->
    Logging.error_f
      ~section
      "Failed to process instrument data: %s"
...
let connect_and_subscribe () : unit Lwt.t =
  Logging.info
    ~section
    "WebSocket instrument stream not yet implemented; using REST API fallback";
  Lwt.return_unit
;;
...
(** Rounds [qty] to the nearest valid quantity step for [info]. *)
...
let validate_order info ~qty ~price =
  ...
  if rqty < info.qty_min
  then Error (Printf.sprintf "Quantity %.8f below minimum %.8f" rqty info.qty_min)
  else if cost < info.cost_min
  then Error (Printf.sprintf "Order cost %.5f below minimum %.5f" cost info.cost_min)
  else if not (is_tradeable info.status)
  then
    Error
      (Printf.sprintf
         "Pair %s not tradeable (status: %s)"
         info.symbol
         (status_to_string info.status))
...
let initialize_symbols symbols : unit Lwt.t =
  Logging.debug_f
    ~section
    "Initializing instrument data for %d symbols"
    (List.length symbols);
