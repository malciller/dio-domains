...
let string_contains str substr =
  ...
  and substr_len = String.length substr in
  ...
  then false
    ...
    let rec loop i =
      ...
      then false
      else if String.sub str i substr_len = substr
      then true
      else loop (i + 1)
    in
    loop 0)
...
let rec json_to_string_precise ?field_name symbol json =
  ...
  | `Intlit s -> s
  | `Tuple _ | `Variant _ ->
    failwith "json_to_string_precise: Tuple and Variant not supported"
;;

let parse_ws_response json : Kraken_common_types.ws_response =
  let method_ =
    Yojson.Safe.Util.(
      member "method" json |> to_string_option |> Option.value ~default:"")
  in
  if String.equal method_ "pong"
  then (
    let req_id = Yojson.Safe.Util.(member "req_id" json |> to_int_option) in
    let time_in =
      Yojson.Safe.Util.(
        member "time_in" json |> to_string_option |> Option.value ~default:"")
    in
    let time_out =
      Yojson.Safe.Util.(
        member "time_out" json |> to_string_option |> Option.value ~default:"")
    in
    { method_
    ; success = true
    ; req_id
    ...
    let success =
      Yojson.Safe.Util.(
        member "success" json |> to_bool_option |> Option.value ~default:false)
    in
    let req_id = Yojson.Safe.Util.(member "req_id" json |> to_int_option) in
    let time_in =
      Yojson.Safe.Util.(
...
let resolve_response req_id (response : Kraken_common_types.ws_response) =
  ...
  Lwt_mutex.with_lock state.mutex (fun () ->
    try
      let wakener, expected_method, _timestamp =
        Response_table.find state.responses req_id
      in
  ...
  | Some (wakener, expected_method) ->
    ...
    then (
      Logging.error_f
        ~section
        "Response method mismatch for req_id=%d: expected '%s', got '%s' (response \
         discarded)"
        req_id
        expected_method
        response.method_;
      ...
      (try Lwt.wakeup_later wakener response with
       | Invalid_argument _ ->
         (* Promise already resolved, typically by a preceding timeout. *)
         Logging.debug_f
           ~section
           "Promise for req_id=%d was already resolved (likely timeout), response \
            discarded"
           req_id);
      Lwt.return_unit)
  | None ->
    Logging.warn_f
      ~section
      "No waiter found for req_id=%d (response arrived after timeout, method=%s, \
       success=%b)"
      req_id
      response.method_
      response.success;
...
let fail_all_pending reason =
  let pending =
    Lwt_mutex.with_lock state.mutex (fun () ->
      let wakers =
        Response_table.fold (fun _ (wak, _, _) acc -> wak :: acc) state.responses []
      in
  ...
  >|= List.iter (fun wak ->
    ...
    | Invalid_argument _ ->
      Logging.debug
        ~section
        "Attempted to wake already-resolved promise during fail_all_pending")
;;
...
let start_reader conn generation =
  let stream =
    Lwt_stream.from (fun () ->
      ...
      else if state.connection_generation <> generation
      then Lwt.return_none
      else
        Lwt.catch
  ...
  let process_frame = function
    | { Websocket.Frame.opcode = Websocket.Frame.Opcode.Close; _ } ->
      Logging.warn_f
        ~section
        "Trading WebSocket closed by server (generation %d)"
        generation;
  ...
  Lwt.async (fun () ->
    ...
    then (
      ...
      Logging.warn_f
        ~section
        "Trading WebSocket connection closed unexpectedly (End_of_file, generation %d)"
        generation;
...
let ensure_connection ?on_failure ?on_connected token =
  Lwt_mutex.with_lock state.mutex (fun () ->
    (match on_failure with
     | Some f -> state.on_failure <- Some f
     | None -> ());
    if state.conn <> None
    then (
  ...
  | `Wait ->
    ...
    let rec wait_for_connection () =
      Lwt_condition.wait state.connection_ready
      >>= fun () ->
      Lwt_mutex.with_lock state.mutex (fun () ->
        if state.conn <> None
        then (
  ...
  | `Connect ->
    ...
    >>= (function
     ...
     | Some (conn, generation) ->
       Logging.debug_f
         ~section
         "Connection established successfully (generation %d)"
         generation;
       start_reader conn generation;
       notify_connection `Connected;
       (* Replay all registered subscriptions on the new connection. *)
       Lwt_mutex.with_lock state.mutex (fun () -> Lwt.return state.subscriptions)
       >>= fun subs ->
       Lwt_list.iter_s
         (fun sub ->
       ...
       Logging.warn_f
         ~section
         "Reader completed (generation %d), connection function returning"
         generation;
       Lwt.return_unit)
;;
...
let start_periodic_tasks () =
  ...
  then (
    let rec run () =
      Lwt_mvar.take cleanup_mvar
      >>= fun () ->
      cleanup_stale_response_entries ~reason:"backpressure" ()
      >>= fun () ->
  ...
  then Lwt.fail_with (Printf.sprintf "Request req_id %d cancelled due to shutdown" req_id)
    ...
    Lwt_mutex.with_lock state.mutex (fun () ->
      ...
      | None ->
        Logging.warn_f
          ~section
          "Cannot send request req_id=%d: WebSocket not connected"
          req_id;
        Lwt.return (Error (Failure "Trading WebSocket not connected"))
      | Some conn_with_gen ->
        (* Guard against req_id collision before registering the wakener. *)
        if Response_table.mem state.responses req_id
        then (
          Logging.error_f
            ~section
            "Request ID collision detected: req_id %d already in use"
            req_id;
          Lwt.return
            (Error
               (Failure
          ...
          then (
            Logging.warn_f
              ~section
              "Response table size is high: %d pending requests"
              pending_count;
            request_cleanup ());
          Lwt.catch
            (fun () ->
      ...
      >>= (function
       ...
       | `Timeout ->
         ...
         Lwt_mutex.with_lock state.mutex (fun () ->
           ...
           else Lwt.return `Already_removed)
         >>= fun removal_status ->
         (match removal_status with
          | `Was_pending ->
            ...
            Logging.warn_f
              ~section
              "Request timeout: req_id=%d, timeout_ms=%d, sent %.3fs ago"
              req_id
              timeout_ms
              timeout;
            Lwt.cancel waiter;
...
let send_request ~symbol ~method_ ~params ~req_id ~timeout_ms
  : Kraken_common_types.ws_response Lwt.t
  =
  let message =
    `Assoc [ "method", `String method_; "params", params; "req_id", `Int req_id ]
  in
  let message_str = json_to_string_precise symbol message in
  send_message ~message_str ~req_id ~expected_method:method_ ~timeout_ms
...
let subscribe message =
  Lwt_mutex.with_lock state.mutex (fun () ->
    let has_channel =
      match extract_channel_name message with
      | Some _ -> true
      | None -> false
    in
    if has_channel
    then
      ...
      <- message
         :: List.filter
              (fun existing -> not (is_same_channel message existing))
              state.subscriptions
    else if not (List.mem message state.subscriptions)
    then state.subscriptions <- message :: state.subscriptions;
    match state.conn with
    | Some conn_with_gen ->
    ...
    | None ->
      Logging.info_f
        ~section
        "Subscription registered, will be sent when connected: %s"
