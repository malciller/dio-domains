...
let write_execution_event buffer event = RingBuffer.write buffer event
...
let update_open_order_price ~symbol ~order_id ~new_price =
  ...
  (match Hashtbl.find_opt store.open_orders order_id with
   ...
   | None ->
     Logging.debug_f
       ~section
       "EAGER_PRICE_UPDATE %s [%s] not in cache, skipping (new_price=%.8f)"
       order_id
       symbol
       new_price);
  Mutex.unlock store.orders_mutex
...
let start_periodic_tasks () =
  ...
  then (
    let rec run () =
      ...
      Logging.debug_f
        ~section
        "Running Kraken queue maintenance (%s)"
        (match reason with
         | `Signal -> "requested"
...
let update_open_orders store (event : execution_event) =
  ...
  let remaining_qty = if raw_remaining_qty <= 0.0 then 0.0 else raw_remaining_qty in
  let epsilon =
    ...
    let ( merged_limit_price
      ...
      | Some prev ->
        ...
        let rq =
          ...
          then remaining_qty
          else if prev.remaining_qty > 1e-12
  ...
  (match prev_price_qty with
   | Some (prev_lp, prev_oq, prev_rq)
     ...
     let stored_lp =
       ...
       | None -> event.limit_price
     in
     let stored_oq =
       match Hashtbl.find_opt store.open_orders event.order_id with
       | Some o -> o.order_qty
       | None -> event.order_qty
     in
     let stored_rq =
       match Hashtbl.find_opt store.open_orders event.order_id with
     ...
     if price_changed || qty_changed || rq_changed
     then
       Logging.debug_f
         ~section
         "CACHE_OVERWRITE %s [%s] exec_type=%s status=%s: price %s->%s | qty %.8f->%.8f \
          | remaining %.8f->%.8f"
         event.order_id
         event.symbol
       ...
       Logging.debug_f
         ...
         (string_of_exec_type event.exec_type)
   | _ -> ());
  if is_terminal_status || is_effectively_filled
  then
    if was_tracked
    then (
      if is_terminal_status
  ...
  | `Removed_terminal ->
    Logging.debug_f
      ~section
      "Removed order: %s [%s] status=%s exec_type=%s"
      event.order_id
      event.symbol
      (string_of_order_status event.order_status)
      (string_of_exec_type event.exec_type)
  | `Removed_zero_qty rq ->
    Logging.debug_f
      ~section
      "Removed order with zero remaining qty: %s [%s] status=%s exec_type=%s \
       remaining=%.12f qty=%.12f"
      event.order_id
      event.symbol
      (string_of_order_status event.order_status)
      (string_of_exec_type event.exec_type)
  ...
  | `Terminal_untracked ->
    Logging.debug_f
      ~section
      "Terminal event for untracked order: %s [%s] status=%s"
      event.order_id
      event.symbol
      (string_of_order_status event.order_status)
  | `Filled_untracked ->
    Logging.debug_f
      ~section
      "Ignored effectively filled order for untracked id: %s [%s] status=%s"
      event.order_id
      event.symbol
      (string_of_order_status event.order_status)
  | `Upserted (was_present, needs_cleanup, remaining_qty) ->
    if needs_cleanup
    then trigger_stale_order_cleanup ~reason:"open_orders_exceeds_1000" ();
    if was_present
    then
      Logging.debug_f
        ~section
        "Updated open order: %s [%s] %.8f@%.2f (filled: %.8f/%.8f) status=%s"
        event.order_id
        event.symbol
        remaining_qty
      ...
      Logging.debug_f
        ~section
        "Added new open order: %s [%s] %s side %.8f@%.2f status=%s"
        event.order_id
        event.symbol
    ...
    then
      Logging.debug_f
        ~section
        "Trade fill: %s [%s] qty=%.8f price=%.2f (total filled: %.8f/%.8f)"
        event.order_id
        event.symbol
  ...
  try
    ...
    | Some sym ->
          ...
          (match existing_order with
           ...
           | None ->
             (match symbol_side with
              ...
              | _ -> "buy"))
        (* Default to buy when no prior data exists. *)
      in
      let order_qty =
              ...
              | None ->
                (match existing_order with
                 ...
                 | None ->
                   Logging.debug_f
                     ~section
                     "FIELD FALLBACK order_qty=0.0 for %s [%s] exec_type=%s status=%s \
                      (no cached order)"
                     order_id
                     (Option.value symbol_opt ~default:"?")
                     exec_type_str
                     order_status_str;
                   0.0)))
      in
           ...
           | None ->
             (match existing_order with
              ...
              | None ->
                Logging.debug_f
                  ~section
                  "FIELD FALLBACK limit_price=None for %s [%s] exec_type=%s status=%s \
                   (no cached order)"
                  order_id
                  (Option.value symbol_opt ~default:"?")
                  exec_type_str
      ...
      let fee =
        ...
        | None ->
          (try
             ...
             match fees with
             | fee_obj :: _ -> parse_float_opt fee_obj "qty"
             | [] -> None
           with
           | _ -> None)
      in
      ...
      then
        Logging.debug_f
          ~section
          "Processing minimal event for order %s [%s]: status=%s exec_type=%s"
          order_id
          sym
          order_status_str
          exec_type_str;
...
let handle_snapshot json on_heartbeat =
  try
    ...
    Logging.debug_f
      ~section
      "Processing execution snapshot with %d items"
      (List.length data);
    ...
    then (
      Logging.debug_f
        ~section
        "Reconciling open orders: removing %d stale orders not present in snapshot"
        removed_count;
      List.iter
        (fun (symbol, order_id) ->
           let store = get_symbol_store symbol in
  ...
  | exn ->
    Logging.error_f
      ~section
      "Failed to process execution snapshot: %s"
      (Printexc.to_string exn)
;;
...
let handle_update json on_heartbeat =
  ...
  | exn ->
    Logging.error_f
      ~section
      "Failed to process execution update: %s"
      (Printexc.to_string exn)
;;
  ...
  try
    ...
    (match channel with
     ...
     | _ ->
       Logging.debug_f
         ~section
         "Executions message received: channel=%s, type=%s, method=%s"
    ...
    | Some "executions", Some "snapshot", _ -> handle_snapshot json on_heartbeat
    | Some "executions", Some "update", _ -> handle_update json on_heartbeat
    | Some "heartbeat", _, _ -> on_heartbeat ()
    | _, _, Some "subscribe" ->
      let success = member "success" json |> to_bool_option in
      (match success with
       | Some true -> Logging.debug_f ~section "Subscribed to executions feed"
       | Some false ->
       ...
       | None -> ())
    | Some "status", _, _ -> Logging.debug ~section "Status message received"
    | _ -> ()
  with
