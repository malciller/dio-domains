  ...
  Lwt.async (fun () ->
    Lwt_mutex.with_lock responses_mutex (fun () ->
      ...
      then
        Logging.info_f
          ~section
          "Failing %d pending response waiters (reason: %s)"
          count
          reason;
      Response_table.iter
...
let cleanup_stale_responses () =
  Lwt_mutex.with_lock responses_mutex (fun () ->
    ...
    then
      Logging.debug_f
        ~section
        "Cleaned up %d stale response table entries"
        (List.length !stale);
    Lwt.return_unit)
;;
    ...
    then (
      ...
      (try
         ...
         then (
           ...
           try Lwt_condition.broadcast pong_condition () with
           | _ -> ());
         ...
         then (
           let log_msg =
             if String.length frame.Websocket.Frame.content > 1000
             then String.sub frame.Websocket.Frame.content 0 1000 ^ "... [TRUNCATED]"
         ...
         let is_response =
               ...
               Lwt_mutex.with_lock responses_mutex (fun () ->
                 ...
                 | Not_found ->
                   ...
                   then
                     Logging.debug_f
                       ~section
                       "Received 'post' response for unknown req_id %d: %s"
                       id
           ...
           | None ->
             ...
             then
               Logging.warn_f
                 ~section
                 "Received 'post' message without identifiable ID: %s"
                 (Yojson.Safe.to_string json);
             false
         in
         ...
         then (
           ...
           then
             if channel = "webData2"
             then Logging.debug ~section "Broadcasting webData2 data message"
             else Logging.debug_f ~section "Broadcasting data message: channel=%s" channel;
...
let send_request ~json ~req_id ~timeout_ms =
  ...
  Lwt.pick
    ...
    ; (Lwt_unix.sleep timeout
       >>= fun () ->
       Lwt_mutex.with_lock responses_mutex (fun () ->
         if Response_table.mem responses req_id
         then (
           Response_table.remove responses req_id;
           Lwt.return `Timeout)
         else Lwt.return `Resolved)
       >>= function
       | `Timeout ->
