...
let parse_fee_info body_str =
  ...
  try
    ...
    let spot_maker =
      ...
      | None ->
        (try
           ...
           match
             member "spotAdd" schedule |> to_string_option |> Option.map float_of_string
           with
           | Some _ as r -> r
           | None -> Some 0.0004
         with
    ...
    let spot_taker =
      let user_rate =
        member "userSpotCrossRate" json |> to_string_option |> Option.map float_of_string
      in
      match user_rate with
      | Some _ -> user_rate
      | None ->
        (try
           let schedule = member "feeSchedule" json in
           match
             member "spotCross" schedule |> to_string_option |> Option.map float_of_string
           with
           | Some _ as r -> r
           | None -> Some 0.0007
         with
    ...
    Logging.debug_f
      ~section
      "Retrieved Hyperliquid fees: perp_maker=%s, perp_taker=%s, spot_maker=%s, \
       spot_taker=%s"
      (Option.map (fun f -> Printf.sprintf "%.4f%%" (f *. 100.)) maker
       |> Option.value ~default:"None")
      (Option.map (fun f -> Printf.sprintf "%.4f%%" (f *. 100.)) taker
       |> Option.value ~default:"None")
      (Option.map (fun f -> Printf.sprintf "%.4f%%" (f *. 100.)) spot_maker
       |> Option.value ~default:"None")
      (Option.map (fun f -> Printf.sprintf "%.4f%%" (f *. 100.)) spot_taker
       |> Option.value ~default:"None");
    Some
  ...
  | exn ->
    Logging.error_f
      ~section
      "Failed to parse Hyperliquid fee response: %s (JSON: %s)"
