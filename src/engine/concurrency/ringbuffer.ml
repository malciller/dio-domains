...
let clear buffer =
  Dio_memory_tracing.Memory_tracing.Tracked.Array.iter (fun atomic -> Atomic.set atomic None) buffer.data;
...
(** Destroy the ring buffer and free its memory *)
let destroy (buffer : 'a t) : unit =
