...
let json_of_market_data exchange symbol base_asset quote_currency =
  ...
  | Some (module Ex) ->
    ...
    in
    let quote_balance =
      try Ex.get_total_balance ~asset:quote_currency with
      | _ -> 0.0
    in
    let pos =
    ...
    let bids_json =
      ...
      | None ->
        (match tob with
         | Some (b, bs, _, _) -> `List [ `Assoc [ "price", `Float b; "qty", `Float bs ] ]
         | None -> `List [])
    in
    let asks_json =
      match ob_event with
    ...
    in
    let trades_json =
      if String.equal exchange "alpaca"
      then (
    ...
    `Assoc
      [ ( "bid"
        , match tob with
          | Some (b, _, _, _) -> `Float b
          | None -> `Null )
      ; ( "ask"
        , match tob with
          | Some (_, _, a, _) -> `Float a
          | None -> `Null )
      ; ( "tob_bid"
        , match tob with
          | Some (b, _, _, _) -> `Float b
