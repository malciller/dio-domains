(* Oracle classes - risk-class definitions and pooled curve estimation
   (Phase 2).
   ...
   MFD samples of all its member series (Oracle_mfd.samples), reusing the
   ...
   table, and the vol-normalized z-index (Oracle_classes.z_index_of) that
   drives the kappa blend: the class contributes F_class^z evaluated at each
   asset start's own vol regime (Oracle_replay.blended_f), so a low-vol
...
let member_bars (s : series) =
  s.bars |> Oracle_calendar.sort_bars |> Oracle_calendar.dedup
;;
...
let pooled
  ...
  List.iter
    (fun s ->
       ...
       let xs = Oracle_mfd.samples ~closes ~lows ~horizon ~warmup ~stride () in
...
(** Pooled class z-CDF, precomputed as a sorted weighted z-array with prefix
    ...
    coverage bisection). [stride] mirrors Oracle_mfd.samples. *)
type z_index =
  ...
  List.iter
    (fun s ->
       ...
       while !i <= Array.length closes - 1 do
         (match Oracle_stats.z_mfd ~closes ~lows ~s:!i ~horizon ~w:vol_window with
          | Some z -> zs := z :: !zs
          | None -> ());
         i := !i + stride
       done;
