(* Oracle stats - realized volatility, normalized MFD, and the asset/class
   blend.
   ...
   trailing vol) plus the class's pooled z-CDF (Oracle_classes.z_index_of):
   the class contribution is evaluated at each asset start's own vol regime. *)

let percentile = Oracle_math.percentile
let std = Oracle_math.std
...
let z_mfd ~closes ~lows ~s ~horizon ~w =
  match Oracle_mfd.mfd ~closes ~lows ~start:s ~horizon with
  | None -> None
  | Some m ->
...
(** Per-start (MFD, trailing-vol) pairs over [warmup, n-horizon-1], shared by
    ...
    window) gets sigma = 0.0; the z-blend (Oracle_replay.blend_index_of)
    excludes those starts from both sides of the blend, since they carry no
    volatility information. [stride] mirrors Oracle_mfd.samples. *)
type asset_regime =
  { mfd : float array
  ; sigma : float array
  ; n : int
