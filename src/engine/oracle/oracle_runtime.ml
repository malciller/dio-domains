(* Oracle_runtime - event-driven capital allocation across venues.
   ...
   One pass re-resolves every configured asset: all-time merged history ->
   references -> runway -> survival-replay parameter search -> decision
   record, sized against per-venue pools walked in config presentation order
   (first = highest priority; capacity passes down; a lower-priority strategy
   is never starved while quote exists). Pools are per venue and never cross
   venues.

   Hot path is pure arithmetic over in-process state:
   - history is cached and refreshed on the [refresh_seconds] background poll;
   - balances come from the websocket-fed live store, else the last fetched
     snapshot, adjusted in process by fill/cancel deltas (notify_fill /
     notify_order_cancel), so events never wait on network;
   - live prices read the in-process top-of-book with a sane-band check
     against the last bar close.

   Events (fills, cancels, balance changes, explicit requests) coalesce into
   one re-resolve.

   Strategy-facing contract: four values per asset ([active],
   [grid_interval], [buy_qty], [sell_qty]), generated by the pass below and
   consumed everywhere. Values are emitted for inactive strategies too, for
   visibility, pool accounting and reactivation eligibility. Normalization
   happens at the exchange layer. [reason], [d_surv], [regime] and [branch]
   are diagnostics for logs and dio-oracle output only. *)
...
(** One asset's live trading decision for the domain workers. The contract is
    the four sizing/activity fields; the rest is observability. *)
type decision =
  { exchange : string
  ; symbol : string
  ...
  ; exhaustion_price : float
    (** The price of the deepest rung fillable with available capital - the
        point where the ladder's quote exhausts. For active strategies this
        is the venue-simulated [P_funded] matching the reported d_surv; for
        inactive ones the single-asset replay's exhaustion point. *)
...
(** Domain-safe wake channel for [request_pass].
    ...
    [notify_order_cancel] run inside the execution cycle); [wait_until] runs
    on the main Lwt domain. [Lwt_condition] is not safe off the Lwt domain: a
    worker would race the scheduler on Lwt's unsynchronized waiter list and a
    wake could run the oracle continuation on the trading domain's stack,
    inflating execution latency. Instead a self-pipe (as in
    [Strategy_common.OrderSignal]) lets a non-blocking byte write from any
    domain wake the Lwt read without touching Lwt internals. *)
...
(** Lwt-side wake fan-out, touched only on the main Lwt domain: the pump
    fiber below drains the self-pipe and broadcasts this condition; every
    [wait_until] waits on it. Domain workers never touch Lwt.

    A self-pipe read fd is single-reader: two concurrent
    [Lwt_unix.wait_read] waiters (pass loop and refresh loop both call
    [wait_until]) could race to consume one coalesced byte, leaving the loser
    parked to its deadline. Funnelling wakes through one pump fiber and
    broadcasting a condition gives all-waiter wake semantics without Lwt
...
(** One traversal of an asset's open orders yielding all quantities the pass
    needs: resting-buy existence, quote committed to resting buys, and base
    tied in resting sells. One pass instead of three [fold_open_orders]
    walks. *)
...
(** Per-account balance view for one pass: live-store snapshot when present,
    else the cached snapshot, plus the epoch its in-process deltas were
    measured against. Built once per account rather than 2-4 times per asset
    (each rebuild is O(balances x symbols)). *)
...
let run_account_pass
  ...
  | Some acs ->
    ...
    (* Open-order summaries and config bounds are invariant across the pass,
       so resolve them once here instead of 2-3x in the sizing loop. *)
    let task_rows =
      List.map
        (fun (t : Oracle_tasks.task) ->
    ...
    List.iter
      (fun ( (t : Oracle_tasks.task)
         ...
         (* No usable data yet: publish nothing rather than a fabricated
            zero inactive decision, which would open the domain's startup
            gate on values never computed. The asset joins the surface on
            the next pass after refresh delivers history and balances. *)
           ...
           then skip_asset "history has no usable close"
             ...
             | Some o ->
               ...
               (* Funded strategies tie their next buy's quote; starved ones
                   pass capacity down. Resting buys are already committed in
                   the pool, so only fresh active placements consume free
                   quote. *)
    ...
    let built_rev =
      List.map
        (fun ((t, d, need, current, outcome) :
           ...
           if d.active
           then (
             (* Active strategies report the venue-simulated survival: the
                shared-capital drawdown sim in priority order. Its P_funded
                is the exhaustion price matching that d_surv. *)
        ...
        built_rev
    in
    (* Cancellation cascade: a starved strategy (survival met but next buy
        does not fit) cancels lower-priority resting buys until its need
        fits. The trigger's own orders are never cancelled. If no combination
        fits, resolution proceeds to the next-highest priority and capacity
        stays with its owners. Cancelled strategies re-evaluate on the cancel
        event this pass triggers, resuming iff quote covers their buy. *)
