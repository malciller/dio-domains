...
let definition_of_tasks (tasks : Oracle_tasks.task list) : definition =
  { positions =
      List.map
...
let timeline_of_series (series : Oracle_types.series list) =
  let dates =
    List.fold_left
      (fun dates (value : Oracle_types.series) ->
         Array.fold_left
           (fun dates bar -> bar.Oracle_types.date :: dates)
           dates
           value.bars)
      []
      series
...
let align_series (timeline : string array) (series : Oracle_types.series) =
  let by_date = Hashtbl.create (Array.length series.bars) in
  Array.iter (fun bar -> Hashtbl.replace by_date bar.Oracle_types.date bar) series.bars;
...
let to_pool_key (key : instrument_key) : Oracle_portfolio.pool_key =
  { venue = key.venue; quote = key.quote; testnet = key.testnet }
;;

let to_portfolio_transfer (transfer : transfer_spec) : Oracle_portfolio.transfer =
