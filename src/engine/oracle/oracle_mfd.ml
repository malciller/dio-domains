(* Oracle MFD - maximum fractional drawdown and its empirical CDF/survival.
   ...
   All windows are index-space over a session series built by Oracle_calendar.
   The no-lookahead sigma invariant lives in Oracle_stats: volatility windows
   only use bars <= s.
...
let f_h ~closes ~lows ~horizon ~threshold ~warmup ?(stride = 1) () =
  ...
  then
    invalid_arg
      (Printf.sprintf
         "Oracle_mfd.f_h: empty distribution for horizon %d warmup %d: no valid start \
...
(** Oracle surface for one horizon across drawdown thresholds. *)
