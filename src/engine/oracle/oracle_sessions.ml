(* Oracle sessions - expected-market-session model (Phase 2).
...
let business_weekday : model =
  let is_session d =
    let w = Oracle_calendar.iso_wday d in
    w >= 1 && w <= 5
  in
...
let expected_sessions (m : model) ~(from_date : string) ~(to_date : string) =
  Oracle_calendar.dates_between ~from_date ~to_date |> List.filter m.is_session
;;

(** Expected sessions skipped by a bar series. *)
let missing_sessions (m : model) ~(bars : bar array) =
  Oracle_calendar.missing_sessions ~is_session:m.is_session bars
;;
...
let gaps_of_missing (m : model) ~(bars : bar array) =
  Oracle_calendar.gaps_of_missing ~bars (missing_sessions m ~bars)
;;

(** Full detect_gaps for a series under this model. *)
let detect_gaps (m : model) (bars : bar array) =
  Oracle_calendar.detect_gaps ~calendar_kind:Equity ~is_session:m.is_session bars
;;
