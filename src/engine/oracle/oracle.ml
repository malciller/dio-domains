(* DIO Capital Oracle - facade.
   ...
   history (Oracle_classes), the kappa blend (Oracle_stats.blend), and the
   equity expected-session model (Oracle_sessions) drive equity gap
   detection. Strategy path replay / historical path coverage live in
   Oracle_replay (generic) over the strategy models in Oracle_strategy;
   live-engine wiring stays out of scope (Phase 3). *)
...
type config =
  ...
  ; equity_sessions : Oracle_sessions.model option
...
let asset_samples ~(series : series) ~horizon ~warmup ?(stride = 1) () =
  let bars = series.bars |> Oracle_calendar.sort_bars |> Oracle_calendar.dedup in
  let closes = Array.map (fun b -> b.close) bars in
  let lows = Array.map (fun b -> b.low) bars in
  Oracle_mfd.samples ~closes ~lows ~horizon ~warmup ~stride ()
;;

(** Blended percentile table for one horizon: invert the unified z-blend
    (Oracle_replay.blended_f on a non-overlapping stride model) per
  ...
  let rows =
    List.map
      (fun p ->
         { percentile = p
         ; mfd =
             Oracle_replay.d_for_coverage
               ~f:(fun d -> Oracle_replay.blended_f model ~d)
               ~target:(p /. 100.0)
         })
      percentiles
  in
...
let estimate_class
  ...
  List.iter2
    (fun (h : horizon) (asset_surface : survival_surface) ->
       ...
       in
       let class_table =
         Oracle_classes.class_percentile_table
           ~weight_by_sessions:config.weight_by_sessions
           ~members:class_input.members
           ~horizon:h
       ...
       let blended_rows =
         List.map
           (fun (ar : surface_row) ->
              let f = Oracle_replay.blended_f model ~d:(ar.drawdown_pct /. 100.0) in
...
let analyze series config =
  let bars = series.bars |> Oracle_calendar.sort_bars |> Oracle_calendar.dedup in
  let is_session = Option.map Oracle_sessions.is_session config.equity_sessions in
  let gaps =
    Oracle_calendar.detect_gaps ~calendar_kind:series.calendar_kind ?is_session bars
  in
  let max_gap = Oracle_calendar.max_gap gaps in
  if max_gap > config.gap_tolerance
  then
    failwith
      (Printf.sprintf
         "Oracle.analyze: %s has %d-session gap(s) (tolerance %d); refusing to \
