(* DIO Capital Oracle - facade.
   ...
   backwards-only) -> references (max drawdown / ATH / ATL) -> runway math
   (floor price, regime, aggressiveness) -> survival replay + parameter search
   -> four-field decision record per asset.

   Oracle_fetch     all-time merged per-asset series, disk-cached.
