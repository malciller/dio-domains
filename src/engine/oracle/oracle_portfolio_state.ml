(* Small, explicit persistence format for the oracle's portfolio allocations.
...
type position =
  { key : Oracle_topology.instrument_key
  ; pool : float
  ; base : float
  }
  ...
  | Some venue, Some symbol ->
    ...
    (match pool, base with
     | Some pool, Some base
       ...
       Ok { key = Oracle_topology.key ~venue ~symbol ~testnet (); pool; base }
