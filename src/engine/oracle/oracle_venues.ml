(* Oracle_venues - resolves each asset's venue metadata (price tick, lot
   ...
   This module is a pure consumer of [Exchange_intf.Oracle.Registry]: venue
   registration is owned by the BINARIES (bin/main.ml / bin/oracle.ml
   force-reference the venue impl modules, which register BOTH the
   live-trading modules and the oracle data-venue adapters at load time -
   see the force-ref blocks there). Under OCaml's unit-level dead-code
   elimination, top-level registry-register side effects only run when the
   modules are referenced, and that reference now lives where the link
   closure is known.
   ...
   implement Exchange_intf.Oracle.S, register it, and be linked from the
   binaries to participate:
