(** Structured logging system with ANSI formatting, per-section level filtering, and
    domain-safe asynchronous output.

    Line layout (aligned): HH:MM:SS.mmm LVL SECTION message 19:17:23.672 INFO
    oracle_runtime [2/8] hyperliquid/BTC/USDC ACTIVE ... 19:17:23.672 INFO oracle_runtime
    ┆ worst drop 83.6% (peak $19497.40 ...
    - Time-only timestamp, gray; the date is available from `docker logs --timestamps`.
    - Level column fixed at 5 chars, colored by severity.
    - Section column fixed at 20 chars so the message column never shifts, and colored
      with a stable per-section hue.
    - Multi-line messages render continuation lines under a dim ┆ gutter at the message
      column instead of a repeated prefix.
    - Lines are word-wrapped to the terminal width (auto-detected or a fixed
      `logging_width` override). Non-terminal output uses `COLUMNS` when set, else 200, so
      wrapped lines keep the gutter without being cramped.
    - With colors disabled the layout and alignment are preserved without ANSI escapes.

    Hot-path contract:
    - All levels except CRITICAL are formatted and pushed to an async queue; the caller
      performs zero I/O and never drains the queue.
    - The single background drain thread owns every write and flush, at ~50ms cadence
      (DEBUG/INFO) or ~1ms while a WARN/ERROR requests prompt flush.
    - A log line costs roughly a microsecond and a handful of allocations on the caller
      (timestamp, colored line, queue push), independent of buffered lines. *)

(* OxCaml marks [Domain.DLS] and [Unix.putenv] as [unsafe_multidomain].
   - [Domain.DLS] caches the last section lookup per domain, storing a shared [section]
     record whose [min_level] may be mutated via [set_section_level]. [Domain.Safe.DLS]
     requires a portable stored value, which this record is not; the record is only
     mutated during configuration changes.
   - [Unix.putenv] is called once at startup by [load_dotenv], before any domain is
     spawned. *)
...
(* Per-section identity colors for the most frequent sections, keeping the interleaved
   components mutually distinct. The oracle_* family shares bright-yellow so a block of
   oracle lines reads as one subsystem. Unlisted sections fall back to a stable hash into
...
(* Palette excludes the severity colors (31 red, 32 green, 33 yellow) so the level column
   keeps its meaning. *)
...
(* ---- Line width ---- [configured_width] overrides all (None = auto). Auto mode detects
   ...
   are picked up without an ioctl per line. Non-terminal output (pipes, `docker logs`,
   files) falls back to `COLUMNS` when set, else [default_width]; wrapping still applies. *)
...
(* Width hint for non-terminal output (e.g. `COLUMNS=200 app | less`); [None] when unset
   or malformed, so [default_width] applies. *)
let env_width () =
...
(* Always [Some] in practice: [configured_width], a detected terminal, COLUMNS, or
   [default_width]. Wrapping is always on. *)
let current_width () =
...
(** Serializes output_channel writes across domains to prevent interleaved log lines from
    concurrent workers. *)
...
(* Format the current wall-clock time as "HH:MM:SS.mmm" (time only; the date is available
   from `docker logs --timestamps`). Caches the time prefix per second via Atomic for
...
(* ---- Column alignment ---- Level column width 5; section column width 20 (fits every
   section that logs in practice, including dashboard_server, domain_supervisor,
   discord_notifier, hyperliquid_startup). Both fixed so the message column never shifts
   mid-stream: every message starts at the same column, which makes the log scannable. A
   section longer than 20 runs on. Multi-line messages render continuation lines under a
   dim ┆ gutter at the message column (see [render_message]). *)
...
(* ---- Word wrapping ---- Wrap [text] at word boundaries so no physical line exceeds
   [width] columns. Multi-space runs are collapsed; an over-long word is emitted on its
...
(* Gutter glyph for continuation lines of a multi-line message; rendered dim. Blank lines
   in a block are dropped. *)
let gutter_glyph = "┆"

(* Split a message into physical lines. The first is the header; the rest are
   detail/gutter lines with caller-embedded leading whitespace stripped. Lines are
   word-wrapped to the terminal width ([width] = [None] leaves them unwrapped); the header
   gets [width - prefix], continuation lines two columns less for the gutter. *)
...
(* ---- Async log drain (all levels except CRITICAL) ---- Hot path: format the message and
   push onto async_queue under async_mutex. Cost ~50ns (mutex + Queue.push); zero I/O and
   no output_mutex contention.

   Background drain thread: writes all queued messages to output_channel, batching
   flushes. The thread owns every flush, so no caller does I/O. It idles at 50ms cadence,
   dropping to ~1ms while a WARN/ERROR requests prompt flush, so urgent lines appear
   within ~ms without blocking the caller.

   CRITICAL: drains the async queue first (preserving order), then writes synchronously
   with flush; the one emergency path allowed to block. *)

let async_queue : string Queue.t = Queue.create ()
let async_mutex = Mutex.create ()
let async_drain_started = Atomic.make false

(* Set when a WARN/ERROR line is queued and should be flushed promptly. Cleared by the
   drain thread; set by any domain. *)
...
let drain_async_queue () =
  ...
  then Mutex.unlock async_mutex
    ...
    (* Transfer pending messages out of the async queue in O(1), minimizing async_mutex
       hold time so producers can push immediately after unlock. *)
    let batch = Queue.create () in
    Queue.transfer async_queue batch;
    Mutex.unlock async_mutex;
    (* Write the batch under output_mutex with one flush per batch: a flush per line cost
       N write plus N flush syscalls per burst. Batch-flush keeps lines equally prompt
       (the whole batch lands this iteration) at one write call each and a single flush. *)
...
(** Start the background drain thread; called once, idempotent. Uses [Thread.create] (not
    [Domain.spawn]) to avoid consuming a core. The thread owns all output flushing, so no
    caller performs I/O. While [flush_requested] is set it polls at ~1ms so WARN/ERROR
    lines surface promptly; else 50ms. *)
...
(* Core logging function. Domain-safe. All levels except CRITICAL are pushed to the async
   queue with no synchronous I/O and no caller-side draining. CRITICAL drains the queue
...
let log_sync level section_name message =
  ...
  then ()
    ...
    if level = CRITICAL
    then (
      (* Emergency path: drain the async queue first to preserve ordering, then write with
         immediate flush. *)
      drain_async_queue ();
      Mutex.lock output_mutex;
      try
      ...
      (* Async path for every other level (including WARN/ERROR): buffer the formatted
         line. The drain thread flushes within ~50ms, or ~1ms for WARN/ERROR via
         flush_requested. *)
      start_async_drain ();
...
(* Format-string log API. Zero-allocation when disabled: [Printf.ifprintf] consumes format
   arguments without allocating a string, and [Printf.ksprintf] allocates only when the
   message is emitted. *)
...
(** Deferred (non-blocking) sibling of [critical_f].
    ...
    Formats at CRITICAL but enqueues on the async drain queue with a prompt-flush request,
    as WARN/ERROR do, instead of draining and flushing synchronously under [output_mutex].
    Use from latency-sensitive paths (e.g. an order-fill handler inside a domain's
    execution cycle): a full or slow output pipe would otherwise block the caller in
    [flush] for the duration of the backpressure. The line still reaches the output within
    ~1ms. *)
...
(** Minimal [.env] loader. When [path] exists, every [KEY=VALUE] line is exported into the
    process environment. Blank lines and [#] lines are skipped, a leading [export ] is
    ignored, and matching single/double quotes around a value are stripped. Existing
    variables are never overwritten. A missing file is a no-op. *)
