(** Centralized error handling for the Dio Domains trading system.
    ...
    Provides unified error classification, exponential backoff retry, Lwt exception
    wrappers, and string utilities. *)
...
(** Semantic error categories, used to determine retry eligibility and logging severity. *)
type error_kind =
...
let classify (err : string) : error_kind =
  ...
  then OrderRejected (* Fallback *)
  else Unknown
;;

(** [true] for retryable kinds: transient transport failures, timeouts, rate limits, and
    server errors. *)
...
(** Retry [f] with exponential backoff per [config], stopping when [max_attempts] is
    reached or the error is not retriable.
    ...
    @param is_retriable_override
      Optional retriability classifier; defaults to [is_retriable_error]. Callers can
      extend the default logic here. *)
...
(** Wrap [f ()] in [Lwt.catch], log exceptions at ERROR with [section] and [operation]
    context, and return [Error msg]. *)
...
(** Like [catch_and_log] but returns [unit], swallowing the error after logging. For
    fire-and-forget operations such as reconnection attempts and background cleanup. *)
...
(** Catch exceptions, log them, then run [recovery]. For reconnect-on-failure and similar
    recovery patterns. *)
