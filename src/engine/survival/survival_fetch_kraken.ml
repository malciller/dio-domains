(* Survival_fetch_kraken - historical daily OHLC for crypto via the Kraken
...
let parse_candle (j : Yojson.Safe.t) : Survival_types.bar option =
  ...
  try
    ...
    in
    Some
      { Survival_types.date = unix_to_iso time
      ; open_ = num 1
      ; high = num 2
      ; low = num 3
      ; close = num 4
...
let parse_ohlc ~(symbol : string) (json : Yojson.Safe.t) : Survival_types.bar list =
  let open Yojson.Safe.Util in
  ...
  then
    failwith
      (Printf.sprintf
         "Survival_fetch_kraken.parse_ohlc: %s errors %s"
         symbol
...
let fetch_ohlc ?(since = 0L) ~(symbol : string) () : Survival_types.bar list Lwt.t =
      ...
      let fetch =
        ...
        then
          Lwt.fail
            (Failure
               (Printf.sprintf
                  "Survival_fetch_kraken: HTTP %d for %s: %s"
                  status
                  symbol
                  body_str))
        else (
