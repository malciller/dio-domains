(* Survival_fetch_alpaca - historical daily bars and the market calendar for
...
let auth_headers () =
  ...
  then
    failwith
      "Survival_fetch_alpaca: ALPACA_API_KEY / ALPACA_API_SECRET not set (add to .env)";
...
let parse_bars (json : Yojson.Safe.t) : Survival_types.bar list =
...
let model_of_calendar_dates (calendar_dates : string list) : Survival_sessions.model =
  Survival_sessions.alpaca_model calendar_dates
;;
...
let regular_session_model () : Survival_sessions.model =
  Survival_sessions.business_weekday
;;
...
let series_of_bars ~(symbol : string) (bars : Survival_types.bar list)
  ...
  ; calendar_kind = Survival_types.Equity
      ...
      let fetch =
        ...
        then
          Lwt.fail
            (Failure
               (Printf.sprintf
                  "Survival_fetch_alpaca: HTTP %d for %s: %s"
                  status
                  symbol
                  body_str))
        else (
...
let fetch_calendar ~(start_date : string) ~(end_date : string) () : string list Lwt.t =
  ...
  then
    Lwt.fail
      (Failure
         (Printf.sprintf "Survival_fetch_alpaca: calendar HTTP %d: %s" status body_str))
