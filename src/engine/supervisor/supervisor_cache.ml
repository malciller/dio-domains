...
let safe_broadcast_update_condition cache =
  ...
  | exn ->
    Logging.warn_f
      ~section:"supervisor_cache"
      "Error broadcasting update_condition: %s"
...
let force_update () =
  ...
  | exn ->
    ...
    let backoff_seconds =
      min 60.0 (2.0 *. (2.0 ** float_of_int (min cache.consecutive_failures 5)))
    in
    cache.backoff_until <- now +. backoff_seconds;
    Logging.warn_f
      ~section:"supervisor_cache"
      "Failed to update connection snapshots: %s"
      (Printexc.to_string exn)
;;
