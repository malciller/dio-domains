...
let circuit_breaker_allows_connection = Supervisor_connection.circuit_breaker_allows_connection
...
let start_monitoring () =
  ...
  let (configs_with_fees, _auth_token) = Lwt_main.run (Supervisor_feeds.initialize_feeds ()) in
