...
(** Mutable state of a single supervised connection: health metrics,
    circuit breaker status, and reconnection bookkeeping. *)
type supervised_connection =
  ...
  ; mutable last_data_received : float option (** Data freshness timestamp; drives heartbeat timeout detection. *)
  ; mutable last_ping_sent : float option (** Timestamp of the last ping; drives ping/pong liveness. *)
...
(** Snapshots the connection list under [registry_mutex], then applies [f]
    to the snapshot outside the critical section. *)
