...
let initialize_feeds () : (Dio_engine.Config.trading_config list * string) Lwt.t =
  ...
  let kraken_symbols =
    app_configs
    |> List.filter (fun cfg ->
      Dio_exchange.Exchange_intf.Types.exchange_of_string cfg.Dio_engine.Config.exchange
      = Kraken)
  ...
  let hyperliquid_testnet =
    match
      app_configs
      |> List.find_opt (fun (cfg : Dio_engine.Config.trading_config) ->
        Dio_exchange.Exchange_intf.Types.exchange_of_string cfg.exchange = Hyperliquid)
    with
  ...
  let ibkr_testnet =
    match
      app_configs
      |> List.find_opt (fun (cfg : Dio_engine.Config.trading_config) ->
        Dio_exchange.Exchange_intf.Types.exchange_of_string cfg.exchange = Ibkr)
    with
    | Some cfg -> cfg.testnet
    | None -> true
  in
  ...
  if has_hyperliquid
  then Hyperliquid.Module.Hyperliquid_impl.set_testnet hyperliquid_testnet;
  (* Apply testnet flag to IBKR module; must happen before gateway connection *)
  if has_ibkr then Ibkr.Module.Config.set_testnet ibkr_testnet;
  if has_alpaca
  then (
  ...
  Logging.info_f
    ~section
    "Connecting to %d Kraken websockets..."
    (List.length kraken_symbols);
  if has_hyperliquid
  then
    Logging.info_f
      ~section
      "Connecting to %d Hyperliquid websockets..."
      (List.length hyperliquid_symbols);
  if has_ibkr
  then
    Logging.info_f
      ~section
      "Connecting to IBKR gateway for %d symbols..."
      (List.length ibkr_symbols);
  if has_lighter
  then
    Logging.info_f
      ~section
      "Connecting to Lighter L2 for %d symbols..."
      (List.length lighter_symbols);
  if has_alpaca
  then
    Logging.info_f
      ~section
      "Connecting to Alpaca WS for %d symbols (%s feed)..."
      (List.length alpaca_symbols)
      alpaca_data_feed;
        ...
        (fun () ->
           let on_failure reason =
             ...
             (* Immediately schedule reconnection; avoids waiting for
               monitor loop backoff (mirrors Kraken auth WS pattern). *)
             Lwt.async (fun () ->
               Lwt.catch
                 (fun () ->
                    Lwt.pause ()
  ...
  let%lwt () =
    if has_hyperliquid
    then Hyperliquid.Module.initialize_instruments_ws ()
    else Lwt.return_unit
  in
  let%lwt () =
    if has_lighter
    then Lighter.Module.initialize_instruments ~symbols:lighter_symbols
    else Lwt.return_unit
  in
  ...
  let%lwt auth_token =
    if has_kraken
    then Kraken.Kraken_generate_auth_token.get_token ()
  ...
  let all_assets =
    ...
    |> List.map (fun symbol ->
      ...
      then String.split_on_char '/' symbol |> List.hd
      else symbol)
    |> List.sort_uniq String.compare
    |> fun assets -> "USD" :: assets
  in
  (* Include USD as quote currency *)
  ...
  let () =
    ...
    | exn ->
      Logging.error_f
        ~section
        "Failed to initialize balances feed stores: %s"
        (Printexc.to_string exn)
  in
    ...
    let ibkr_connect_fn () =
      Lwt.catch
        (fun () ->
           ...
           request_open_orders fires; this avoids a dependency cycle in the lib. *)
           ...
           Lwt.catch
             ...
             is closed. Disconnect cleanly and handle based on market hours. *)
               ...
               so start_async is triggered by the supervisor's on_disconnect handler. *)
                  ...
                  (* Market is open but contract resolution still failed; this is a genuine error.
    ...
    then (
      ...
      let%lwt trading_client_ready =
        ...
        let rec wait_loop () =
          ...
          else if Kraken.Kraken_trading_client.is_connected ()
          then Lwt.return true
          else Lwt_unix.sleep 0.1 >>= fun () -> wait_loop ()
        in
        wait_loop ()
      in
      if not trading_client_ready
      then
        Logging.warn
          ~section
          "Timeout waiting for trading client connection, continuing anyway..."
      else Logging.info ~section "✓ Trading client connected and ready";
      ...
      let%lwt executions_ready =
        Kraken.Kraken_executions_feed.wait_for_execution_data kraken_symbols 10.0
      in
      if not executions_ready
      then
  ...
  let%lwt () =
    if has_kraken
    then (
      let%lwt orderbook_ready =
        Kraken.Kraken_orderbook_feed.wait_for_orderbook_data kraken_symbols 10.0
      in
      if not orderbook_ready
      then
        Logging.warn ~section "Timeout waiting for orderbook data, continuing anyway..."
      else Logging.info ~section "✓ Orderbook feed ready";
      Lwt.return_unit)
    else Lwt.return_unit
  in
  ...
  let%lwt hl_executions_ready =
    if has_hyperliquid
    then Hyperliquid.Executions_feed.wait_for_execution_data all_hyperliquid_symbols 10.0
    else Lwt.return_true
  in
  let%lwt executions_ready =
    if has_kraken
    then Kraken.Kraken_executions_feed.wait_for_execution_data kraken_symbols 10.0
    else Lwt.return_true
  in
  ...
  let%lwt balances_ready =
    let kraken_p =
      if has_kraken
      then Kraken.Kraken_balances_feed.wait_for_balance_data all_assets 10.0
      else Lwt.return_true
    in
    let hl_p =
      if has_hyperliquid
  ...
  then Logging.warn ~section "Timeout waiting for balance data, continuing anyway..."
  else Logging.info ~section "✓ Balances feed ready";
  ...
  let%lwt global_hl_fees =
    ...
    then (
      ...
      | None ->
        Logging.error
          ~section
          "Fatal: Failed to fetch global Hyperliquid fees on startup. Exiting...";
        exit 1)
         ...
         try
           ...
           Express as fraction of trade value for Fee_cache compatibility. *)
             ...
             Dio_strategies.Fee_cache.store_fees
               ...
               ~taker_fee:taker
               ~ttl_seconds:86400.0;
             (* Fees don't change often for IBKR *)
             Lwt.return
               { asset with
                 Dio_engine.Config.maker_fee = Some maker
               ; Dio_engine.Config.taker_fee = Some taker
           ...
           in the instruments feed; no separate fee endpoint is needed. *)
             let fees =
