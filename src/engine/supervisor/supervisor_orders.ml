...
type strategy_callbacks =
  ...
  ; on_amend_ok :
      strategy_order -> string (* target_order_id *) -> string (* new_order_id *) -> unit
  ; on_amend_skipped : strategy_order -> string (* target_order_id *) -> unit
  ; on_amend_fail :
      strategy_order -> string (* target_order_id *) -> string (* error *) -> unit
...
let grid_callbacks : strategy_callbacks =
  ...
  ; on_amend_ok =
      (fun order target_order_id new_order_id ->
        Logging.info_f
          ~section
          "✓ Order amended successfully: %s %s %.8f @ %s New Order ID: %s"
          (side_str order)
          order.symbol
          order.qty
          (price_str order)
...
let mm_callbacks : strategy_callbacks =
  ...
  ; on_cancel_ok =
      (fun order target_order_id ->
        Logging.info_f ~section "✓ Cancelled order: %s" target_order_id;
        Dio_strategies.Market_maker.Strategy.cleanup_pending_cancellation
          order.symbol
          target_order_id)
  ; on_cancel_fail =
      (fun order target_order_id ->
...
let hedger_callbacks : strategy_callbacks =
  { on_place_ok =
      (fun order order_id ->
        Logging.info_f
          ~section
          "✓ Hedger order placed successfully: %s %s %.8f @ %s (Order ID: %s)"
          (side_str order)
          order.symbol
          order.qty
          (price_str order)
  ...
  ; on_place_fail =
      (fun order err ->
        Logging.error_f
          ~section
          "✗ Hedger order placement failed: %s %s %.8f @ %s - %s"
          (side_str order)
          order.symbol
          order.qty
          (price_str order)
...
let process_single_order
  ...
  try
    ...
    let process_fn () =
      ...
      | Place -> dispatch_place ~auth_token ~orders_placed ~cb order
      | Amend ->
      ...
      | Cancel ->
        (match order.order_id with
         ...
         | None ->
           Logging.error_f
             ~section
             "Cancel request missing target order ID for %s"
             order.symbol)
    in
    let reject_fn err =
      match order.operation with
    ...
    then process_fn ()
      ...
      Logging.warn_f
        ~section
        "Exchange %s not connected, dropping order %s %s"
        order.exchange
        (side_str order)
        order.symbol;
    ...
    then Lwt.return_unit
      ...
      then (
        ...
        | exn ->
          Logging.error_f
            ~section
            "Exception in order processing loop: %s"
            (Printexc.to_string exn);
          Lwt.async loop;
          Lwt.return_unit))
  in
