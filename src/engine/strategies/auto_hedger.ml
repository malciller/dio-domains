  ...
  then (
    ...
    let hedge_qty_final =
      ...
      then (
        ...
        then (
          ...
          Logging.info_f
            ~section
            "Testnet: bumping hedge qty from %.8f to %.8f to meet $%.0f min (price=%.2f)"
            filled_qty
            min_qty
            min_value
            fill_price;
    ...
    let order_type, limit_price =
      ...
      | Some (best_bid, _bid_sz, best_ask, _ask_sz) ->
        let px =
          match hedge_side with
          | Sell -> best_bid
          | Buy -> best_ask
        in
        Logging.info_f
          ~section
          "Using perp TOB for %s %s: bid=%.4f ask=%.4f -> limit=%.4f"
          (string_of_order_side hedge_side)
          hedge_symbol
          best_bid
          best_ask
          px;
      ...
      | None ->
        Logging.info_f
          ~section
          "No perp orderbook for %s, falling back to market order"
          hedge_symbol;
        "market", None
    in
    let order =
    ...
    let price_str =
      ...
      | Some p -> Printf.sprintf "limit=%.4f" p
      | None -> "market"
    in
    Logging.info_f
      ~section
      "Hedge %s order (%s %.8f %s %s) pushed to ring-buffer"
      order_type
      (string_of_order_side hedge_side)
      hedge_qty_final
      hedge_symbol
  ...
  then ()
    ...
    | Buy ->
      ...
      then
        Logging.info_f
          ~section
          "Spot buy fill: %.8f %s @ %.4f -> hedge already open (%.8f short), skipping"
          filled_qty
          hedge_symbol
          fill_price
          (get_hedge_qty ())
        ...
        Logging.info_f
          ~section
          "Spot buy fill: %.8f %s @ %.4f -> opening hedge short"
          filled_qty
          hedge_symbol
          fill_price;
        let qty =
          build_and_push_hedge testnet hedge_symbol Sell filled_qty fill_price perp_tob
        in
        if qty > 0.0
        then (
          set_hedge_open qty;
    ...
    | Sell ->
      ...
      then (
        ...
        Logging.info_f
          ~section
          "Spot sell fill: %.8f %s @ %.4f -> closing full hedge short (%.8f)"
          filled_qty
          hedge_symbol
          fill_price
          close_qty;
        let qty =
          build_and_push_hedge testnet hedge_symbol Buy close_qty fill_price perp_tob
        in
        if qty > 0.0
        then (
          clear_hedge ();
        ...
        Logging.info_f
          ~section
          "Spot sell fill: %.8f %s @ %.4f -> no hedge open, skipping"
          filled_qty
          hedge_symbol
          fill_price)
;;
