...
(** Price key: [price * 10000] rounded to int. Shared by the persisted-sell matching in
    [sync_open_orders] and the reconcile in [evaluate_sell_leg]. An int key keeps
    within-tolerance prices in the same (or an adjacent) bucket without allocating a
    string per lookup. *)
...
(** 1-to-1 multiset match between persisted sell levels and open sell orders. Returns
    (open_levels, missing_levels).

    Buckets open orders by tolerance-rounded price key (Hashtbl) and verifies the original
    tolerance before consuming a candidate: ~O(n+m) instead of the prior O(n*m)
    rescan-per-level. Buckets are keyed by an int (price scaled to 4 decimals, rounded),
    not a [Printf.sprintf "%.4f"] string, which allocated per open order and persisted
    level on every execution.

    Fallback for direct [evaluate_sell_leg] callers; the strategy hot path builds the same
    split during [sync_open_orders]' scan and threads it through, so the per-tick
    reconcile is O(m). *)
let partition_persisted_sell_levels persisted open_orders =
  (* Rounded price key -> (price, remaining count) list. Tolerance is checked per
     candidate. *)
...
(** Tolerance used to treat two persisted rung prices as the same level: 1 bp of price or
    1e-4 absolute, matching [partition_persisted_sell_levels]. *)
...
(** Collapse persisted levels whose prices fall within the same tolerance bucket to a
    ...
    the live (feed-matched) entries ahead of the stale/missing ones -
    [open_levels @ kept_missing] - so the surviving rung carries the live qty and the
    stale duplicate from an amend (cancel+replace) window is dropped. This is what stops
    the 1-to-1 reconcile from rewriting the recorded qty back and forth between duplicates
    on every scan (the "0.26 -> 2.34 -> 0.26" flapping). Returns the input list physically
    unchanged when there is nothing to drop, so the scan's index cache is not invalidated
    on the common no-change pass. *)
let dedupe_persisted_sell_levels levels =
  (* The common steady-state list is already strictly price-descending with one rung per
     tolerance bucket, which is exactly the fixed point of the sort+merge below. Detect
     that (linear, no allocation) and return the input physically unchanged, skipping the
     [List.sort] the old code always paid - the sort is charged even when it produces an
     identical list. *)
...
let evaluate_buy_leg
    ...
    iter_open_orders (fun order_id _ _ side_str userref_opt ->
      ...
      then (
        (* An amend on Alpaca is cancel+create under the hood: while it is in flight the
           ...
           trips the ">1 buys" branch below. Cancelling the old id then races the amend
           (the cancel is ignored or bounced), so skip orders that are mid-amendment - the
           amend replaces them. *)
        let is_mid_amend =
  ...
  then (
    ...
    (* A fresh buy must respect the same 2*gi spacing below the closest resting sell that
       ...
       The companion sell the sell leg will place later in this same tick is not yet
       visible here, and in a falling market it lands a rung below the closest existing
       sell. Clamping only against the feed let the fresh buy pass, then the companion
       sell's zone caught it and the next tick amended it down. The prospective sell is
       computed with the shared [owed_sell_price] and folded into the same clamp. *)
    ...
    (* Only anticipate the companion sell when the sell leg is actually owed one WITH
       ...
       rest at [owed_sell_price] and the 2*gi clamp is real. Gating on those signals keeps
       a below-market buy from being pulled down against a sell that will never place
       (e.g. no sellable inventory), which would only add an up-amend on the next tick. *)
    let buy_price =
      ...
      then
        ...
        then (
          ...
          then (
            ...
            state.tif_recovery_pending <- false;
            (* A fresh buy is placed at the current sizing target, so any pending
               re-anchor is satisfied. *)
          ...
          then
            ...
            then (
              (* The balance snapshot is stale: the local figure may be wrong, so the
                 attempt is still worthwhile - the exchange's verdict is the truth. Mark
                 the attempt as knowingly under-funded so the (expected) rejection does
                 not latch capital_low on a foreordained outcome. *)
              Logging.warn_f
                ~section
    ...
    then (
      ...
      | Some (_sell_order_id, sell_price), Some current_buy_price, Some buy_order_id ->
        (* The 2*gi separation from the closest sell is anchored on the sell order and is
           ...
           the buy cross a sell that still exists. The clamp is released only when the
           sell is removed from tracking (fill/cancel/expiry); only then does the buy
           trail the top of book at the grid interval. [sell_price] is always a positive
           resting-order price, so there is no zero-reference hazard. *)
        ...
        (* A sizing re-anchor (the capital oracle published a changed grid interval -
           ...
           buy in both directions. A downward amendment is warranted only by a
           sell-spacing violation (see below); a widened grid interval no longer snaps a
           valid resting buy down to the market rung. The ladder spacing is enforced where
           it matters: fresh placements clamp below the closest sell, and this leg
           corrects real intrusions into a sell's restricted zone. A qty-only oracle
           change does not re-anchor the price: the grid adopts the new size (Alpaca qty
           mismatch amend) or on the next placement, and the resting price only trails up. *)
        ...
        (* Downward movement of the buy is initiated to correct an actual violation of the
           2x grid_interval restricted zone below the closest sell (above sell - 2*gi).
           This clamp is price-independent and enforces the threshold whenever a resting
           buy sits inside the restricted zone (e.g. after an external upward amendment,
           book dislocation, or grid interval widening). A resting buy already outside the
        ...
        then (
          ...
          then (
            ...
            (* An amend replaces the resting buy (cancel+create on Alpaca): the capital
               ...
               affordability check must add the committed notional ([locked_in_buys], sum
               of price*qty over open buys) back to the available balance. Without this
               the grid falsely reports "Insufficient quote balance" when trailing a
               funded buy up on committed capital. *)
