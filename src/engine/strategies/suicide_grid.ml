...
let persistence_accumulation_exchange = Suicide_grid_config.persistence_accumulation_exchange
...
let accumulation_sell_allowed_on_recovery = Suicide_grid_reservation.accumulation_sell_allowed_on_recovery
...
let get_pending_orders max_orders =
  LockFreeQueue.read_batch order_buffer max_orders
...
module Strategy = struct
  ...
  let rec cleanup_strategy_state symbol =
    ...
    if StringMap.mem symbol map then
      let new_map = StringMap.remove symbol map in
      if not (Atomic.compare_and_set Suicide_grid_types.strategy_states map new_map) then
  ...
  let set_startup_replay_done symbol =
    ...
    if state.startup_replay then begin
      ...
      if state.last_fill_oid = None
         ...
         && Suicide_grid_config.persistence_accumulation_exchange state.exchange_id then begin
