(** Fee Cache
    ...
    TTL-based in-memory cache for maker and taker fees, keyed by (exchange, symbol). Uses
    lock-free Atomics for read paths to eliminate hotpath contention across strategy
    domains. *)
...
(** Tracks pending fetch promises keyed by "exchange|symbol" to deduplicate concurrent
    requests. *)
...
(** Removes entries whose TTL has expired from [map] and returns the filtered map. *)
let evict_expired map =
  let now = Unix.time () in
  StringMap.filter (fun _ entry -> now -. entry.timestamp < entry.ttl_seconds) map
;;

(** Looks up the cached maker fee for [(exchange, symbol)]. Returns [Some fee] on a valid
...
let store_fees ~exchange ~symbol ~maker_fee ~taker_fee ~ttl_seconds =
  ...
  Logging.debug_f
    ~section
    "Cached fees for %s: maker=%.6f, taker=%.6f (TTL: %.0fs, cache size: %d)"
    key
    maker_fee
    taker_fee
    ttl_seconds
    (StringMap.cardinal (Atomic.get fee_cache))
;;

(** Asynchronously refreshes the fee for [symbol] on Kraken. No-op if a valid entry
    already exists. Deduplicates concurrent calls via the [in_flight] table. Uses linear
    backoff (0.5s increments, up to 3 attempts) on nonce errors. *)
let refresh_async symbol =
  ...
  then (
    ...
    let promise =
      match existing_promise with
      | Some p ->
        Logging.debug_f
          ~section
          "Fee fetch for %s already in-flight, reusing promise"
          symbol;
        p
      | None ->
        let rec fetch_with_retry attempt =
          Lwt.catch
            (fun () ->
              ...
              match fee_opt with
              | Some
                  { maker_fee = Some maker_fee; taker_fee = Some taker_fee; exchange; _ }
                ->
                store_fees
                  ~exchange
                  ~symbol
                ...
                Logging.info_f
                  ~section
                  "Fetched and cached fees for %s: maker=%.6f, taker=%.6f (attempt %d)"
                  symbol
                  maker_fee
                  taker_fee
                  attempt;
              ...
              | Some { maker_fee = Some maker_fee; taker_fee = None; exchange; _ } ->
                ...
                Logging.info_f
                  ~section
                  "Fetched and cached maker fee for %s: %.6f (taker=default to maker) \
                   (attempt %d)"
                  symbol
                  maker_fee
                  attempt;
              ...
              | Some { exchange; _ } ->
                Logging.debug_f
                  ~section
                  "No fee data available for %s/%s (attempt %d)"
                  exchange
                  symbol
                  attempt;
                Lwt.return_unit
              | None ->
                Logging.debug_f
                  ~section
                  "Failed to fetch fee for %s (attempt %d)"
                  symbol
                  attempt;
                Lwt.return_unit)
            (fun exn ->
              ...
              let is_nonce_error =
                String.length error_msg >= 12
                && String.sub error_msg 0 12 = "Invalid nonce"
              in
              if is_nonce_error && attempt < 3
              then (
                let backoff = float_of_int attempt *. 0.5 in
                Logging.warn_f
                  ~section
                  "Nonce error fetching fee for %s (attempt %d), retrying in %.1fs: %s"
                  symbol
                  attempt
                  backoff
                  error_msg;
                Lwt_unix.sleep backoff >>= fun () -> fetch_with_retry (attempt + 1))
              else (
                Logging.warn_f
                  ~section
                  "Error fetching fee for %s (attempt %d): %s"
                  symbol
                  attempt
                  error_msg;
                Lwt.return_unit))
