(** Frame-clock animation toolkit: monotonic clock, easing, per-key tweening, and decaying
    event flashes.

    [motion_pending] tells the render loop whether another high-rate frame is worth
    drawing: setters mark motion while a value is still moving, and the loop resets the
    flag at the start of each frame. *)

(** Monotonic seconds since program start; does not jump backwards on NTP adjustment,
...
(** Set by [tween]/[flash] while a value is still moving; the loop resets it before each
    draw so it reflects that frame's remaining motion. *)
...
(** Seconds since the previous frame, clamped to [0, 0.1]. Set by [reset_frame]; every
    [tween] in a frame shares one delta time. *)
...
(** Decaying event flash: [1.0] while [active], then exponential decay with time constant
    [tau]. *)
...
(** Called once at the start of each frame: clears [motion_pending] and advances
    [current_dt]. *)
let reset_frame () =
...
(** Time of the last fresh data. Ambient motion (breathing fills, live badges) runs only
    for a short window after a snapshot, so the UI does not animate while the stream is
    idle or the session is detached. *)
...
(** Square-wave blink: [true] for the [duty] fraction of each [hz] cycle; steady-on under
    [reduced_motion]. The near-fill proximity cue: a solid tint reads as "highlighted but
    static", so the highlight must blink. Callers set [motion_pending] while the condition
    holds so the loop keeps redrawing. *)
