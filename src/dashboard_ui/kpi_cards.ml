...
let render_card_row w cards =
  ...
  List.iteri (fun i (title, r1, r2) ->
    ...
    let dashes = I.string A.(fg c_border ++ bg c_bg) (String.concat "" (List.init dash_count (fun _ -> "─"))) in
    ...
    let c_r1 = I.hcat [ I.string A.(bg c_bg) " "; I.hsnap ~align:`Left iw r1; div_mid ] in
    let c_r2 = I.hcat [ I.string A.(bg c_bg) " "; I.hsnap ~align:`Left iw r2; div_mid ] in
    body_row1_imgs := !body_row1_imgs @ [ c_r1 ];
    body_row2_imgs := !body_row2_imgs @ [ c_r2 ];

    (* Bottom bar piece *)
    let bot_dashes = I.string A.(fg c_border ++ bg c_bg) (String.concat "" (List.init (iw + 1) (fun _ -> "─"))) in
  ...
  I.vcat [
    I.hcat !top_imgs;
    I.hcat !body_row1_imgs;
    I.hcat !body_row2_imgs;
    I.hcat !bot_imgs;
  ]
...
let render_kpi_cards w json =
  ...
  let total_hold_strats, total_accum_strats, active_count =
    List.fold_left (fun (hv_acc, av_acc, cnt) (_sym, data) ->
      ...
      let pending_sell_qty = List.fold_left (fun q s -> q +. (s |?> "qty" |> to_float_d 0.0)) 0.0 sell_orders in
  ...
  let c1_row1 = I.hcat [ col 10 a_dim "NET WORTH"; col_right 12 a_bright (format_usd net_worth) ] in
  let c1_row2 = I.hcat [ col 10 a_dim "ACCUM VAL"; col_right 12 a_cyan (format_usd total_accum_val) ] in
  ...
  let c2_row1 = I.hcat [ col 10 a_dim "STRATEGIES"; col_right 12 a_green (Printf.sprintf "%d active" active_count) ] in
  let c2_row2 = I.hcat [ col 10 a_dim "UPTIME   "; col_right 12 a_text (format_duration uptime ^ " │ " ^ string_of_int (List.length recent_fills) ^ " fills") ] in
  let card2 = ("SYSTEM ENGINE", c2_row1, c2_row2) in

  let lats = match json |?> "latencies" with `Assoc l -> l | _ -> [] in
  let cycle_p50, cycle_p99 =
    List.fold_left (fun (p50_acc, p99_acc) (_sym, metrics) ->
  ...
  let c3_row1 = I.hcat [ col 10 a_dim "CYCLE P50 "; col_right 12 (lat_attr cycle_p50) (format_latency_us cycle_p50) ] in
  let c3_row2 = I.hcat [ col 10 a_dim "CYCLE P99 "; col_right 12 (lat_attr cycle_p99) (format_latency_us cycle_p99) ] in
  ...
  let live_pct = if total_kb > 0.0 then (float_of_int live_kb /. total_kb) *. 100.0 else 0.0 in
  let c4_row1 = I.hcat [ col 10 a_dim "HEAP SIZE"; col_right 12 a_yellow (Printf.sprintf "%d MB" heap_mb) ] in
  let c4_row2 = I.hcat [ col 10 a_dim "LIVE RATIO"; col_right 12 a_green (Printf.sprintf "%.1f%%" live_pct) ] in
  let card4 = ("MEMORY / GC", c4_row1, c4_row2) in

  if w < 100 then
    I.vcat [
      render_card_row w [card1; card2];
      render_card_row w [card3; card4];
    ]
  else
    render_card_row w [card1; card2; card3; card4]
