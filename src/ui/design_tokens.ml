...
module Colors = struct
  ...
  (** Primary text colors *)
  let primary_text = A.white
  let bright_primary = A.(st bold ++ white)

  (** Secondary text colors *)
  let secondary_text = A.gray 2
  let dimmed_text = A.gray 3
  ...
  (** Status colors *)
  let success = A.green
  let success_bright = A.(st bold ++ green)
  let warning = A.yellow
  let warning_bright = A.(st bold ++ yellow)
  let error = A.red
  let error_bright = A.(st bold ++ red)
  let info = A.blue
  let info_bright = A.(st bold ++ blue)

  (** Border colors *)
  let border = A.gray 3
  let border_active = A.cyan
  let border_inactive = A.gray 4
end
