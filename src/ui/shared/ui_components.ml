...
let create_vertical_separator ?(height = 24) ?(char = Borders.(get_border_style ()).vertical) () =
  let rec make_vertical n acc =
    if n <= 0 then acc
    else make_vertical (n - 1) (string ~attr:Colors.border char :: acc)
  in
  vcat (make_vertical height [])
...
let create_progress_bar width percentage =
  ...
  string ~attr:Colors.primary_text bar
...
let create_colored_progress_bar width percentage =
  ...
  let filled_bar = String.make filled '#'
  and empty_bar = String.make (width - filled) '.' in
  let bar = hcat [string ~attr:color filled_bar; string ~attr:Colors.secondary_text empty_bar] in
  bar
...
let create_bordered_panel ~title ?(focused = false) ?max_width ?max_height content =
  ...
  let create_border_line left_char right_char =
    hcat [
      string ~attr:border_attr left_char;
      string ~attr:border_attr right_char;
    ]
  in
...
let create_table_row columns widths alignments =
  let create_cell content width alignment =
    ...
    in
    string ~attr:Typography.body padded_content
  in
  hcat (List.map2 (fun (content, alignment) width ->
  ...
  let create_header_cell content width alignment =
    let padded_content = match alignment with
      ...
      | `Center ->
          ...
          Printf.sprintf "%*s%s%*s" left_pad "" content right_pad ""
    in
    string ~attr:Typography.label padded_content
  in
  hcat (List.map2 (fun (content, alignment) width ->
...
let create_table headers rows column_widths alignments =
  ...
  vcat (header_row :: separator :: data_rows)

(** Create status bar with consistent separators *)
let create_status_bar ?(separator = " │ ") elements =
  let separator_widget = string ~attr:Colors.secondary_text separator in
  let rec build_bar = function
    | [] -> empty
    | [last] -> last
    | hd :: tl -> hcat [hd; separator_widget; build_bar tl]
  in
  build_bar elements
...
(** Create panel hint with focus indication *)
let create_panel_hint _panel text focused =
  if focused then create_highlighted_status text
  else create_secondary_status text
