...
let get_fee_info symbol : fee_info option Lwt.t =
  (* Check if memory tracing is available (defined at function scope for exception handler) *)
  let memory_tracing_available =
    try ignore (Sys.getenv "DIO_MEMORY_TRACING"); true with Not_found -> false in
  ...
  Lwt.catch (fun () ->
    ...
    (* Defer memory tracing during HTTP operations to avoid interference with C libraries *)
    if memory_tracing_available then Dio_memory_tracing.Memory_tracing.defer_allocation_tracking ();
    ...
    if status <> 200 then begin
      if memory_tracing_available then Dio_memory_tracing.Memory_tracing.resume_allocation_tracking ();
      ...
      | _ ->
          ...
          | None ->
              if memory_tracing_available then Dio_memory_tracing.Memory_tracing.resume_allocation_tracking ();
              Logging.warn_f ~section "No pair data found in TradeVolume response for %s" symbol; Lwt.return_none
          | Some pair_key ->
              ...
              | _ ->
                  if memory_tracing_available then Dio_memory_tracing.Memory_tracing.resume_allocation_tracking ();
                  Logging.warn_f ~section "Failed to parse fee values for %s (pair: %s)" symbol pair_key; Lwt.return_none))
    end
  ) (fun exn ->
    if memory_tracing_available then Dio_memory_tracing.Memory_tracing.resume_allocation_tracking ();
