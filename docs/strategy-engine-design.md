...
**Milestone 0 (in progress):** `strategy_trace.ml` (observable trace types + comparison + JSON persistence), `strategy_event_recorder.ml` (per-cycle recording, including **emitted order intents** via a per-symbol hot-path hook), and `strategy_equivalence.ml` (trace diff + a deterministic **replay API**) are implemented with ref-vs-ref identity, state order-insensitivity, divergence, JSON round-trip, and replay-equivalence/divergence tests (`test_strategy_harness.ml`). The domain loop records per-cycle venue open orders, emitted intents, and state inputs (price, bid/ask, base/quote tradeable balances, base balance age, open-orders generation, cycle) under the default-off `strategy_trace` config flag, persisting every 50 busy cycles to `data/strategy_trace_<exchange>_<symbol>.json`; `dio strategy diff <a.json> <b.json>` compares two traces. A **replay driver** (`dio strategy replay <trace>`, `Strategy_replay`) re-feeds a recorded input trace through the reference grid and diffs emitted intents. It is now **verified equivalent** on a live alpaca testnet trace (SMH, 650 cycles, 650 emitted cycles). To reach that, the record captures everything the strategy reads: the **oracle/F&G blend** (`grid_interval`, `grid_qty`, resolved `accumulation_buffer`), the start-of-cycle domain knobs the oracle writes outside the call (`force_buy_reanchor`, `capital_low`), the venue's immediately-sellable base (`get_available_balance_fast`, distinct from the tradeable balance), the real open-order ids (so amends target the recorded ids), plus a full strategy-state snapshot taken at the **first traced cycle**: scalars/options (tracked buy, `reserved_base`/`quote`, position, fill price/qty/oid, flags, in-flight bits, `feed_locked_sell_base`, `cached_qty_increment`, duplicate keys) and collections (`open_sell_orders`, `sell_commitments` incl. `sc_armed`, `pending_orders`, `persisted_sell_levels`, `amend_cooldowns`, `sell_holds_since_balance`, `buy_credits_since_balance`). Venue-available ingestion is injected via a `~set_venue_available` callback (Alpaca's `set_available_balance_for_test`), keeping `Strategy_replay` venue-agnostic. **Order-lifecycle events are now captured and replayed** (this bridged the first corpus gap): fill/cancel/ack/amend (and the REST-path `Failed`/`Rejected`/`Amendment_*`/`Cancel_cleanup`) are normalized to `Strategy_trace.event_obs`, recorded on the domain thread at the handler call sites (`Jacobs_ladder_events.dispatch_event` for REST, `record_exec_event` for the exec ring), attached to the next executed cycle, and re-applied through the same handlers by replay (`Jacobs_ladder.apply_event`) before each cycle's strategy call — so state transitions from fills/cancels/amends are reproduced instead of only `book_update` cycles. Recording context (price/bid/ask/balances/generation/cycle/now/oracle blend) was also made complete. Remaining harness gaps: (a) events drained on a non-executing cycle are carried to the next executed cycle (correct ordering, but a trailing event after the last executed cycle is not re-executed); (b) oracle-driven capital reclamation emits (cancel decisions) are not fed as inputs (domain-owned, shared by both paths, so no live divergence); (c) dual-run capture (reference and candidate fed identical inputs in-process) remains the eventual stronger check.

**Corpus result** (10 assets across Alpaca testnet, Kraken, and Hyperliquid live; recorded with `config_strategy:false`): reference **and** candidate both report **equivalent on all 10** — BOTZ, LIT, REMX, SMH (Alpaca), XMR, ETH, SOL, ADA (Kraken), BTC, HYPE (Hyperliquid). The corpus surfaced and fixed real gaps: replay hardcoded `get_open_orders_generation` (non-remaintain venues took the scan-skip path with a stale cached buy count); the per-cycle oracle-blended `asset` was computed but not threaded into the strategy call (masked on Alpaca where recorded gi equals config gi); `startup_replay`/`attributed_balance_increase` unseeded; the trace dropped the order feed's `user_ref` and the symbol; and `strategies/jacobs_ladder.json` used `"stop": true` on the buy branches, which skipped the sell step whenever a buy fired (`execute_strategy` always runs sell after buy).

**Milestone 3 (in progress):** decision handlers ported (`compute_buy_ref_price`, `owed_sell_price`, `available_base`, `grid_price`); a coarse-wrapper seam (`Strategy_actions_grid.Make`) plus the engine context (`Config_grid_engine`) behind a default-off `config_strategy` flag. **Fine-action decomposition of the top-level orchestration is done and verified:** `execute_strategy`'s orchestration is now expressed as strategy-file steps over reference actions (`grid_prepare` → `grid_cleanup` → `grid_sync` → `grid_refresh_fee` → `grid_guard` → `grid_buy` → `grid_sell`) in `strategies/jacobs_ladder.json`; the engine context implements each by calling the same reference sub-function, and the caller holds `state.mutex` for the whole interpreter cycle. `dio strategy replay [--candidate] <trace>` replays a recorded trace through the reference grid or the interpreter; both report **equivalent** on the live alpaca testnet SMH trace (650 cycles, 650 emitted). The **buy leg is decomposed** as file steps over reference actions: `evaluate_buy_leg` is split into `buy_leg_facts` + `buy_cancel_excess` (multi-buy cancel) + `buy_place_initial` (fresh placement, incl. cooldown/capital-low/crossing-sell/balance branches) + `buy_amend` (trail-up / zone clamp), with the original `evaluate_buy_leg` retained as a thin behavior-identical dispatcher. The file selects the branch via `engine:buy_active/buy_pending/buy_should_cancel/buy_count_zero/buy_count_positive` facts (`grid_buy_gate`/`grid_buy_facts`/`grid_buy_cancel`/`grid_buy_place`/`grid_buy_amend`). The **sell leg is decomposed** into three phases over reference actions: `sell_leg_prepare` (derived sizing/gate facts + persisted-level reconcile/logging), `sell_leg_place` (the gated placement block, incl. restore-vs-owed selection, notional gate, prune, hold arming, persisted-level append), and `sell_leg_finalize` (retry-latch bookkeeping, nothing-to-sell consumption, excess sweep, `resuming_after_balance_flag`). `evaluate_sell_leg` recombines the phases, so the reference entry point is unchanged. The file's `sell` step runs `grid_sell_prepare`/`grid_sell_place`/`grid_sell_finalize`. With the buy leg this completes the decision-procedure decomposition: the shipped `strategies/jacobs_ladder.json` no longer calls any single coarse `grid_cycle`/`evaluate_*` action. Remaining: the harness corpus (fill/cancel/amend races) + canary.
...
|---|---|---|---|
| Hardcoded strategy-name dispatch | `domain_spawner.ml` — `is_grid_strategy` (REMOVED, M2) / `is_mm_strategy` (`strategy = "market_maker" \|\| "MM"`) | grid: done; MM: M4 | grid dispatch is now the bound strategy file; MM remains name-detected until ported. |
...
**Removed so far:** the coarse `grid_cycle`/`grid_buy`/`grid_sell` actions and `Config_grid_engine.run_cycle`; the grid hardcoded-name dispatch (M2); the reference grid live entry point (M3).
...
**Files to delete (owner action; the agent cannot remove files).** None are deletable yet — each is still referenced until its step:
- M4 (after the MM port is verified): `src/engine/strategies/market_maker.ml`.
- M5 (native reimplementation only): `src/engine/strategies/jacobs_ladder/jacobs_ladder_execution.ml`, `jacobs_ladder_events.ml` (and, if fully replaced, `jacobs_ladder_types.ml`, `jacobs_ladder_reservation.ml`, `jacobs_ladder_orders.ml`, `jacobs_ladder_sell_orders.ml`, `jacobs_ladder_config.ml`).
- M6 (post-canary): none at file level — `execute_strategy`/reference-replay are functions; remove them in place.
- Independent (`Auto_hedger` abandoned): `src/engine/strategies/auto_hedger.ml` plus its call sites in `domain_spawner.ml` and `src/engine/supervisor/supervisor_orders.ml`, the `hedge` config field, and the dune entry.
